...
//
//	    **「上階の、自階にもある種別」を offset 0 で指さない。** VW はその形を**自階のレベル**
//	    （＝始端と同じ Z）へ解決してしまい、実体が無い柱になる（実機で 46 本。パス長 2959 の
//	    柱で StartElevation=572 / EndElevation=572。パスは 2 点あり、潰れているのは解決の側）。
//	    実機で切り分けた分かれ目:
//	      * `{上階, 横架材天端, 0}`    … 壊れる（横架材天端は**自階にもある**）
//	      * `{上階, 軒高, 0}`          … 無事（軒高は 2 階に無い）
//	      * `{上階, 横架材天端, 2374}` … 無事（offset が 0 でない）
//	    そこで**上端 offset が 0 になるときだけ上階の FL を基準に取り直す**（`{1, FL, 天端 −
//	    上階のストーリ高さ}`）。上端は上階のレベルに紐付いたままなので**階高の変更に追随する**
//	    性質は保たれ、**絶対 Z も動かない**。上階の横架材天端が FL と同じ高さ（＝その階の
//	    横架材天端オフセットが 0）の階だけは FL へ移しても 0 のままで、**上階を指す言い方が
//	    残らない**——そこは最後の手段として上端も当階のレベルへバインドする（描かれない柱
//	    よりは描かれる柱を採る）。実装は `buildColumnCommands`、経緯は

...
//
//	    **「上階の、自階にもある種別」を offset 0 で指さない。** 実機で実体が無かった 46 本が
//	    そろってこの形だった（パス長 2959 の柱で StartElevation=572 / EndElevation=572。
//	    パスは 2 点あり、潰れているのは解決の側）。同じ図面・同じレイヤ・同じ種別でも、
//	    offset が 0 でなければ正しく上階へ解ける:
//	      * `{上階, 横架材天端, 0}`    … 実体が無かった（横架材天端は**自階にもある**）
//	      * `{上階, 軒高, 0}`          … 無事（軒高は最上階にしか無い＝自階に無い）
//	      * `{上階, 横架材天端, 2374}` … 無事（EndElevation=5905 ＝ 3531 + 2374）
//	    そこで**上端 offset が 0 になるときだけ上階の FL を基準に取り直す**（`{1, FL, 天端 −
//	    上階のストーリ高さ}`）。上端は上階のレベルに紐付いたままなので**階高の変更に追随する**
//	    性質は保たれ、**絶対 Z も動かない**。
//	    **上階の横架材天端が FL と同じ高さの階（＝その階の横架材天端オフセットが 0）はそのまま**
//	    ——FL へ移しても 0 のままで上階を指す言い方が残らず、当階へバインドし直すのは階高への
//	    追随を失う後退だからである。
//	    **これは確定した原因ではなく、いちばん確からしい見立てである**——「手で同じ値を入れ
//	    直すと正しく描かれる」という実機の事実は、record ではなく**生成時の解決**が犯人だとも
//	    読める。図面の側が実際に何を持っているかは `draw/DrawUtil` の `DescribeStoryBound` が
//	    読み戻して診断へ載せる。経緯は docs/DEV-NOTES.md「柱が長さ 0 で描かれる（M27）」。
//
...
namespace HomeskzIfcImport::parse
{
	...
	// 上端 offset を「0 とみなす」許容値（mm）。**上階の、自階にもある種別を offset 0 で指す**
	// と VW が終端を始端と同じ Z へ解決し、実体が無い柱になる（ヘッダ冒頭「『上階の、自階にも
	// ある種別』を offset 0 で指さない」）。丸めのぶんだけ見る。

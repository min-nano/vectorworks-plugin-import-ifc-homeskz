...
//	  * **高さはパスのジオメトリ（下端の絶対 Z ＋ 柱高さ）で確定させ、上下端をストーリ
//	    レベルへバインドする**（横架材と同じ仕組み。**部材の実体を作るのはパスで、バインドは
//	    高さの基準を与えるだけ**——M8 のローカル確認で、バウンドだけを正しくしても長さ 0 の
//	    ままだと何も描かれないことを確かめた。draw/Column.cpp 冒頭）。柱（管柱・通し柱）は
//	    下端を当階・上端を上階（storyOffset=1）の横架材天端（最上階は軒高）へ、小屋束は
//	    上下端とも当階の横架材天端へバインドする。
//	    **小屋束の上端 offset は下端と同値**にする: VW の構造材ツールは上下端バウンドの
//	    offset 差（上 − 下）をパス由来の部材長へ**加算**するため、パスが既に柱高さを持つ
//	    小屋束で上端 offset を天端相当（下端 + 高さ）にすると柱高さが二重加算され上端が
//	    約 2 倍になる（Python 版 #116 のフォローアップ。柱は上端が上階バインドで offset 差が
//	    ≈0 になるため起きない）。

...
//
//	    **上端が上階のレベルにちょうど届く柱が実体を持たなかった事故（M27）は、高さ基準とは
//	    無関係だった。** 実機（スキップフロアのモデル）で 1 階の管柱 46 本がパス長 2959 なのに
//	    StartElevation=572 / EndElevation=572 になった件で、**原因は渡した 2 点のパスが
//	    PIO 化（`CreateCustomObjectPath`）で潰れること**と確定している（実機 round 7〜10）。
//	    直したのは描画側（潰れていたらパスを作り直して差し替える。`draw/StructuralMember` の
//	    `retryWithFreshPath`）で、**この解析側の書き方は症状とも直し方とも関係が無い**。
//	    **高さ基準は犯人ではない**と実機で確定している（round 5・6）:
//	      * VW は命令どおりに記録している（`GetObjectStoryBound` の読み戻しが一致）。
//	      * 上端を `{上階, FL, −40}` と言い換えても、レイヤの高さ基準（`{レイヤ, 0}` 〜
//	        `{レイヤ, 2959}`）で言い直しても、**実測は 572→572 のまま動かない**。
//	    したがって**高さ基準の書き方を変える直し方はもう試さない**。
//	    **`{1, FL, …}` への書き換えは残してあるが、これは直し方ではない**——絶対 Z を動かさ
//	    ない書き換えで、「上端が上階のレベルにちょうど届く柱」を命令の側から見分けやすいので
//	    そのままにしてあるだけである。経緯は docs/DEV-NOTES.md「柱が長さ 0 で描かれる（M27）」。
//

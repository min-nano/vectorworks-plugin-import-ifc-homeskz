...
//	  * **高さを表すのは上下端バウンドの差だけ**（描画側のパスは平面座標しか持たない。
...
//	    高さ 0 で描かれる——M8 のローカル確認で、同値にした小屋束だけが高さ 0 になり、差が
//	    実高さの管柱は正しく描かれた（docs/DEV-NOTES.md M8）。
//
//	    **上端が上階のレベルにちょうど届く柱が実体を持たなかった事故（M27）は、高さ基準とは
//	    無関係だった。** 実機（スキップフロアのモデル）で 1 階の管柱 46 本がパス長 2959 なのに
//	    実体を持たなかった件は、**渡した 2 点のパスが `CreateCustomObjectPath` で潰れたあと、
//	    潰れ方に 1 ULP の丸めが残って `ResetObject` の再構築から外れていた**のが原因で、
//	    直したのは描画側（**パスに Z を渡さない**＝平面座標だけにする。`draw/Column`）である。
//	    **バウンドの側に落ち度は無い**と SDK リファレンス issue #59 でも確定した。
//	    したがって**高さ基準の書き方を変える直し方はもう試さない**。
//
//	    **一時期ここにあった `{1, FL, …}` への書き換えは外した。** 「`{上階, 横架材天端, 0}`
//	    という形が柱を潰す」という**外れた見立て**に基づく回避策で、絶対 Z は動かさないので
//	    絵の高さは合っていたが、**基準が FL になる**ぶん OIP には「FL −40」と出て、階の
//	    横架材天端オフセットを変えても追随しない柱になっていた（実機で指摘）。

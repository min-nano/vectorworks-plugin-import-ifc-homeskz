...
//
//	    **上下端が「階だけ違う同じ記録」にならないようにする。** レベル種別も offset も同じ
//	    で `storyOffset` だけが違うバウンドを渡すと、VW は**終端を始端と同じ Z へ解決し**、
//	    実体が無い柱になる（実機で実測: パス長 2959 の柱で StartElevation=572 /
//	    EndElevation=572。パスは 2 点あり、潰れているのは解決の側）。上階参照そのものは
//	    効いている——同じ「上階の横架材天端」でも offset が 0 でない通し柱は正しく解けた。
//	    そこで**下端を FL 基準へ振り替えて**記録を違うものにする（上端は上階のレベルに
//	    紐付けたまま＝階高の変更に追随する）。**絶対 Z は動かない**。実装は
//	    `buildColumnCommands` の末尾、経緯は docs/DEV-NOTES.md「柱が長さ 0 で描かれる（M27）」。
...
namespace HomeskzIfcImport::parse
{
	...
	// 上下端のバウンドを「同じ記録」とみなす offset の許容値（mm）。**階だけ違う同じ記録**に
	// なると VW が終端を始端と同じ Z へ解決し、実体が無い柱になる（ヘッダ冒頭「上下端が同じ
	// 記録にならないようにする」）。丸めのぶんだけ見る。

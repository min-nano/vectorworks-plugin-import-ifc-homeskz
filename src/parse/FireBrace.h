...
namespace HomeskzIfcImport::parse
{
	...
	// 要素が火打（Name が "火打" 始まりの IfcBeam / IfcMember）か。
	...
	// 2 線分を**無限直線として延長した**交点を返す。平行なら nullopt。
	std::optional<core::Vec2> segmentIntersection(const Segment2D& first, const Segment2D& second);

	// 火打 footprint の 2 つの端面（梁に取り付く面）のワールド線分を返す。world と local
	// は同じ並びの頂点列で、local の第 2 成分 v が始終点で符号反転する辺（＝中心線をまたぐ辺）
	// を端面とみなす。端面はちょうど 2 つのはずだが、判定はそのまま返し、個数の検査は
	// fireBraceBasePoint が行う。
	std::vector<Segment2D> fireBraceEndFaces(const std::vector<core::Vec2>& world,
											 const std::vector<core::Vec2>& local);

	// 2 つの端面の直線を延長した交点（内角の点＝シンボルの基準点）を返す。端面がちょうど
	// 2 つでない・平行で交点が定まらないときは nullopt。
	std::optional<core::Vec2> fireBraceBasePoint(const std::vector<Segment2D>& faces);

	// 火打の向きに合わせた回転角（度）を返す。基準点（内角）から火打本体の重心へ向かう方向
	// （内角の二等分方向）に kFireBraceAngleOffset を加える。world が空なら 0（呼び出し側は空
	// の外形をここへ渡さない）。
	double fireBraceAngle(const core::Vec2& base, const std::vector<core::Vec2>& world);

	// STEP Model から火打のシンボル配置命令を組み立てる。
	//

...
//	parse/Rafter.h
//
//	Phase 1（IFC 解析）の垂木モジュール。Python 版 ifc/rafter.py に対応する。
//
//	【前提】ホームズ君 EX の IFC には**垂木が一切出力されない**（オブジェクト・型・
//	プロパティのいずれにも垂木の位置や仕様が現れない。Python 版が全フィクスチャで確認済み）。
//	そのため垂木は IFC から抽出できず、要件どおり**屋根版（Name が "屋根版" 始まりの
//	IfcSlab。勾配した平面外形を鉛直に押し出したソリッド＝押し出しが屋根の厚み）の勾配・
//	外形から導出**する。屋根面が建物形状の一次情報で、部材ではなく面が形を決める
...
//
//	導出の要件（Python 版 CLAUDE.md「垂木」節。docs/DEV-NOTES.md M6）:
...
namespace HomeskzIfcImport::parse
{
	...
	// 屋根面の掃引方向の広がり [eMin, eMax] に垂木の掃引位置を割り付ける（Python 版
	// _sweep_positions 相当）。

...
//	だが、**別の 1 通**として後から投稿される。絵を見てから書くには Vectorworks を操作
//	できなければならず、そのためにプラグインは所見を待たずに戻る——尋ねて投稿するのは
//	別プロセスのダイアログ（`scripts/vw-feedback.*` の `ask-note`）の仕事になる
//	（docs/DEV-NOTES.md M23「所見は別プロセスのダイアログで訊く」）。
//

...
{
	// 適用するデータタグスタイル名（Python 版 ifc/tag.py TAG_STYLE）。**文書に用意された
	// スタイルを名前で引くのではなく、この名前を基準に描画側が取り込みのたびに作る**
	// （draw/Tag の createTagStyle。「既存のスタイルに頼らない」＝受け取った文書に何も
	// 用意されていなくても断面寸法が出る）。同じ名前の資源が文書に既にあれば、描画側が
	// "-2"、"-3" … と後ろを足して**ユーザーのスタイルを乗っ取らない**。
	// 作れなかった場合は描画側がスタイル無しで置く（タグを失わない）。

...
//	parse/Tag.h
//
//	Phase 1（IFC 解析）の断面寸法データタグモジュール（docs/DEV-NOTES.md M13）。各横架材の断面
//	寸法（"120×180" 等）を図の上に表示するためのデータタグ命令（core::TagCommand）を組み立てる。
//
...
//
//	【タグは伏図だけでなく軸組図にも載せる】タグを平らに持って「tag['layer'] がその
//	ビューポートの表示レイヤに含まれるか」で振り分ける形は、**軸組図（断面ビューポート）
//	では成り立たない**——断面に映る横架材は「レイヤ」では選べず、**切断面に乗るかどうか**で決ま
//	るからである。そこで振り分けをここ（解析側）で済ませ、タグは
//	**そのビューポートの命令（core::ViewportCommand::tags）の中**に入れる。描画側は
//	「このビューポートの tags を置く」だけになり、伏図と軸組図で 1 つの実装を共有できる。
//
//	【伏図のタグ】表示レイヤに乗る横架材 1 本につき 1 つ。位置は**軸中央から軸直交方向（上また
//	は左）へ断面幅/2 だけ寄せた点＝部材の辺の中央**（左右に伸びる梁は上辺の中央、
//	上下に伸びる梁は左辺の中央）。ここにデータタグの下端中央が来る。**余白を足さず面ちょうどに
//	置く**のは、部材から離すと VW が関連付け先へ引出線を描くため。
//
...
//	伏図（平面ビューポート）は真上から見た図なので投影＝平面座標そのまま。軸組図は横から見た図
//	なので
...
{
	// 適用するデータタグスタイル名。**文書に用意されたスタイルを名前で引くのではなく、
	// この名前を基準に描画側が取り込みのたびに作る**（draw/Tag の createTagStyle。
	// 「既存のスタイルに頼らない」＝受け取った文書に何も用意されていなくても断面寸法が出る）。
	// 同じ名前の資源が文書に既にあれば、描画側が"-2"、"-3" … と後ろを足して**ユーザーの
	// スタイルを乗っ取らない**。作れなかった場合は描画側がスタイル無しで置く（タグを失わない）。
	inline constexpr const char* kTagStyle = "断面寸法";

	// 軸方向 (dx, dy) に沿った読みやすい文字角度（度）を返す。軸の角度を (-90, 90] に正規化し、
	// 文字が上下反転しないようにする。
	double tagAngle(double dx, double dy);

	// 軸 (dx, dy) に直交する単位ベクトルのうち「上または左」を向く側。y が大きい（上）
	// 方を選び、y が同等（材が南北向き）なら x が小さい（左）方を選ぶ。長さが 0 の軸には既定
	// （上＝(0, 1)）を返す。

...
//	**ただし原点は一致しない**（ローカル確認で判明）。実機では**高さは合っているのに横だけ
//	一定量ずれた**——断面の水平座標は世界座標そのものではなく、断面線のどちらかの端から
//	測った距離らしい。**その規約を当てずっぽうで決めると、外したときに逆方向へ同じだけ
//	ずれるだけ**なので、軸組図のタグは**原点に依存しない置き方**にした:
//	  * データタグは関連付けると VW が関連付け先へ吸着させる（＝VW が「関連付け先が注釈
//	    空間のどこに来るか」を計算してくれている）。
//	  * そこから **anchor（横架材の挿入点）→ position（部材の辺の中央）のモデル上の変位**
//	    ぶんだけ動かせば、原点がどこであっても正しい場所へ来る。
//	  * 投影は距離を保つ（切断線に沿った長さと Z はそのまま）ので、この変位は注釈空間でも
//	    同じ長さ・同じ向きになる。
//	したがって sectionAnnotationPoint が返す値は**絶対位置としては使わず、2 点の差**
//	（position − anchor）としてだけ使う。伏図は注釈空間がモデルの平面座標そのものである
//	ことが実証済みなので、従来どおり絶対位置で置く（core/Document.h の TagPlacement）。
//

...
//
//	【現状は未使用】メニューコマンド（Extensions/ExtMenu）は M1 以降 buildDocument の
//	結果（ストーリ・通り芯・床の件数）を表示するので、本サマリは呼ばれていない。
//	Document を経由せず生の Model から「どの IFC 型が何件あるか」を直接数える独立した
//	確認手段として残してあり、M15（実行 UX・診断）で完了文言の組み立てへ転用する
//	（ROADMAP.md M15）。
//

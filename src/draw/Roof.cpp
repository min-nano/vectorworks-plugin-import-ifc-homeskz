...
//	draw/Roof.cpp
//
//	野地板描画の実装。命令セット（RoofCommand）を**屋根面オブジェクト（Roof Face）
//	**として配置する。【SDK 依存】PluginPrefix.h（VectorWorks SDK）を include するため、
//	この翻訳単位はプラグインビルド（SDK あり）でのみコンパイルされ、無 SDK の core/parse
//	ライブラリには入れない（CLAUDE.md「依存の向きは厳守する」）。
//
//	【屋根面オブジェクトを直接組み立てる】ISDK には屋根作成の一連の呼び出し（VectorScript の
//	`BeginRoof` … `EndGroup`）が無い。代わりに**屋根面オブジェクトを外形・高さ・勾配から直接組
//	み立てられる**（VWFC の VWRoofFaceObj。外形ポリゴンから生成し、屋根軸・棟側・勾配・軸の Z
...
namespace HomeskzIfcImport::draw
{
	namespace
	{
		// 勾配（rise/run）を渡すときの run の基準値（mm）。命令の rise/run は屋根面の**単位法
		// 線成分**（rise=水平成分 dh・run=鉛直成分 nz）で、比はそのまま勾配だが、
		// 値そのものは 1 mm 未満の極小値になる。屋根面は rise/run を**長さ（WorldCoord）
		// **として保持するため、極小値のまま渡すと勾配計算が丸めに埋もれるおそれがある。
		// そこで VW 自身のエクスポートと同じ規約に合わせ、比を保ったまま run＝25.4（1 インチ）

...
namespace HomeskzIfcImport::draw
{
	...
	DrawCounts executeDocument(const core::Document& document, core::ProgressReporter& progress)
	{
		...
		// 【重ね順はここでは触らない】床・野地板が柱・梁を覆わないようにする件は
		// **drawSheets が持つ**——まずビューポート単位の重ね順上書きを試し、それが図面に
		// 記録されなかったときだけデザインレイヤの並べ替え（reorderStoryLayers）へ落ちる
		// （draw/Sheet.cpp の冒頭）。図面のレイヤの並びは、退避路へ落ちない限り動かない。

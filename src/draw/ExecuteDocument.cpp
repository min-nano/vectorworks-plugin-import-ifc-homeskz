...
namespace HomeskzIfcImport::draw
{
	...
	DrawCounts executeDocument(const core::Document& document, core::ProgressReporter& progress)
	{
		...
		// 凡例の縮率は**この undo イベントを閉じた後**に当て直す（draw/Legend.h の
		// PendingLegendScale）。そのためスコープの外で受ける。
		...
		// **undo イベントのブロックより外**に置く——凡例の診断はイベントを閉じた後に決まる。
		...
		//
		// ★**入れ子のブロックにしてある**のは、イベントを閉じてから凡例の縮率を当てるため。
		{
			...
			const auto beginPhase = [&](const char* label, std::size_t count, core::DrawPhase phase)
			{
				if (progress.cancelled())
					return false;
				progress.beginPhase(
					label, core::drawPhaseShare(count, phase, weightedTotal, core::kDrawShare),
					count);
				return true;
			};
			...
			drawSymbolPhase("アンカーボルトを配置しています…", "アンカーボルト",
							document.anchorBolts, core::DrawPhase::AnchorBolts, counts.anchorBolts);
			...
			drawSymbolPhase("仕口を配置しています…", "仕口", document.joints,
							core::DrawPhase::Joints, counts.joints);
			...
			if (beginPhase("伏図を作成しています…", document.sheets.size(),
						   core::DrawPhase::Sheets))
			{
				std::string note;
				counts.sheets =
					drawSheets(document, progress, &note, &memberHandles, &pendingLegends);
				addDiagnostics(note);
			}
		...
		// ★**凡例の縮率はイベントを閉じてから当てる。** イベントの内側で当てても、閉じ際に
		// VW が凡例を作り直して既定（1:50）へ戻してしまう（draw/Legend.h の
		// PendingLegendScale・docs/DEV-NOTES.md「グラフィック凡例」）。
		//
		// **「取り消し」の対象からは外れる**が、取り消せば凡例そのものが消えるので実害は無い。
		applyLegendImageScale(pendingLegends.legends, pendingLegends.scale);
		// 当て直しで枠の大きさが変わるので、右上をもう一度揃える（draw/Legend.h）。
		placeLegends(pendingLegends.legends, pendingLegends.topRight);
		addDiagnostics(legendDiagnostics(pendingLegends.legends));

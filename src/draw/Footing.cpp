...
//
//	【立上りの描画手順】
//	  1. CreateWall(始点, 終点, 壁厚) で壁芯から壁を作る。**VectorScript の
...
//
//	構成層の合計＝壁厚なので、立上りの厚みは命令どおりに保たれる（実データの壁厚は 120 / 150
//	/ 300mm と混在するので、厚みの決まった既製スタイルでは合わない）。
//
...
//	  3. **床付け（捨てコン・砕石）のソリッド** … 地中梁の下に敷く層（M17。
...
//	（スラブのモディファイアを扱う API は ModifySlab だけ）。UI では噛み合わせられるので、
...
//	るのと同じ方針。上記「スタイルは作らない」）。見え方はクラス（基礎スラブ）の by-class
//	属性で決まる。
//
...
namespace HomeskzIfcImport::draw
{
	namespace
	{
		...
		// （docs/DEV-NOTES.md M10。取り違えていれば入れ替えるだけ）。
		...
		//
		// 続けて**その下の床付け（捨てコン・砕石）**も同じ押し出しのソリッドとして置く（M17）。
		...
		MCObjectHandle DrawOneWall(const core::WallCommand& wall, bool& outPlaced)
		{
			...
			//
			// 構成は高さバインドより**先に**入れる。構成の変更が壁の属性を作り直しても、
			// 後から入れる高さが残るようにするため。
							  ...
							  const ObjectHandles& handles, std::string* outNote)
	{
		...
		// ——実機で両方が起きた（docs/DEV-NOTES.md M10）。結合を拒否された命令があっても、
		...
		// ResetObject で明示的に作り直す（docs/DEV-NOTES.md M10）。
		...
		// ので、本数や端点の一致を「異常」として出すと平常時から鳴り続ける（docs/DEV-NOTES.md M10）。

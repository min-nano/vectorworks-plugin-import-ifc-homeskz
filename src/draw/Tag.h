...
//	    **中身を入れてから PIO へ渡す**——空のグループを先に渡して後から足すと、VW が渡した
//	    時点でグループを複製している場合に足したテキストが迷子になる（実機で「オブジェクトは
//	    出るのにタグレイアウトが空」として現れた）。渡した後は PIO が実際に持っているものを
//	    取り直して中身の数を確かめ、複製されていればこちらのグループを消す。
//	    レイアウトの入り口は**2 つ**あり（GetCustomObjectProfileGroup と、VW2020 で足された
//	    GetCustomObjectProfileGroupInAux）、スタイルの中の PIO のように図面上に無い
//	    オブジェクトでは後者にしか出ないことがあるので両方見る。
//	  * **スタイルのパラメータ対応表**（SetAllPluginStyleParameters で全部「スタイル依存」に
//	    する）。これが無いとシンボル定義は形だけのスタイルで、当てたインスタンスは自分の値
//	    ＝空の既定レイアウトを使い続ける。実機で「タグは出るのに中身が空」になった原因。
//
//	【実機から持ち帰った見本】ユーザーが手で作った（実際に寸法が出ている）スタイルの構造を
//	診断ダンプで読むと
...
//
//	【データタグの作法（ローカル確認を重ねて定まった順）】CreateCustomObject("Data Tag",
//	挿入点, 角度) → **対象の横架材へ関連付け** →スタイル（skipValidation）→ 引出線を OFF →
//	ResetObject → ビューポート注釈へ追加 →タグを更新 → 実位置と実寸を測る → （全部置いてから）
//	**目標の位置へ動かす**引出線を OFF にするのは、**タグを部材の面ちょうどに置いても既定 ON
//	だと引出線が描かれる**ため（位置の決め方は parse/Tag.h）。関連付け先が無い（構造材ツールで
//	描けずフォールバックの直線になった）横架材のタグは、関連付けだけを省いて置く。
//
...
namespace HomeskzIfcImport::draw
{
	...
	struct TagCounts
	{
		...
		// 置いたタグ 1 本の実際の姿（当たっているスタイル名とレイアウトの中身の数）。
		// **実描画はローカルの VW でしか見られない**ので、「タグはあるのに中身が空」の
		// 原因がスタイル未適用なのかレイアウト空なのかを実機から持ち帰る目。
		std::string firstTag;
	};
	...
	// タグが残る方が原因を追いやすい）。
	//
	// 【診断に構造を出す】実描画はローカルの VW でしか見られないので、診断行には作った
	// スタイルの**中身の構造**（何が入っていて、レイアウトに何件載ったか）を出す。基準名が
	// 埋まっている文書——つまりユーザーが手で作った「断面寸法」スタイルがある文書——では
	// **その既存スタイルの構造も並べて出す**。VW が本当はどこへタグレイアウトを置くのかを
	// 実機から持ち帰る唯一の目で、こちらの作り方と突き合わせられる。

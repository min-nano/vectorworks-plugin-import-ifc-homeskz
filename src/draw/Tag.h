...
//	  タグを更新 → 実位置と実寸を測る → （全部置いてから）**較正して目標へ動かす**
...
//	  2. **タグは指定した挿入点に留まらない**。関連付けると VW が関連付け先へ吸着させる。
//	     そこで**最後に GetObjectBounds で実位置と実寸を測って動かす**（タグの実寸は
//	     スタイルが決めるので、部材から逃がす量も描くまで分からない）。
...
//	     ビューポートを再更新する。
//	  4. **断面（軸組図）の注釈空間は原点がモデルと違う**（高さは合うのに横だけ一定量
//	     ずれた）。規約が分からないので**実測から較正する**——下記 Calibration。
//
...
namespace HomeskzIfcImport::draw
{
	...
	// 注釈空間の原点の較正結果（draw/Tag.cpp の CalibrateAnnotationOrigin）。**断面
	// （軸組図）の注釈空間はモデルの投影と平行移動ぶんだけずれている**が、その量も、
	// VW がタグを吸着させる部材上の基準点も分からないので、置いた実位置から測って決める。
	// 選んだ基準点と残差は診断行へ出す——次のローカル確認で規約そのものが読み取れる。
	struct Calibration
	{

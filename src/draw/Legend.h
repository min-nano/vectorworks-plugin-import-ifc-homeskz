...
//	    縮尺ではない。
//	    ★**書く先はレコードではない。** パラメトリックレコードの `ImageScale` を書く道は
//	    実機で潰れている（書けて読み戻せるのに OIP も実描画も既定の 1:50 から動かない。
//	    docs/DEV-NOTES.md「打ち切った調査」）。代わりに**凡例の中の「凡例イメージ」
//	    オブジェクト**（`kWorksheetImageNode` ＝ 56）を型で探し、ハンドル単位の
//	    `SetWorksheetImageScaleFactor` で与える（下の applyLegendImageScale）。凡例イメージは
//	    中身を流し込んだときに作られるので、**必ず作り直しの後**に呼ぶ。
...
namespace HomeskzIfcImport::draw
{
	...
	//
	// **書き先は凡例 PIO のレコードではなく、凡例の中の「凡例イメージ」オブジェクト**
	// （`kWorksheetImageNode` ＝ 56。凡例イメージはワークシートのイメージと同じ節点型で、
	// ハンドル単位の `Get/SetWorksheetImageScaleFactor` が効く見込み）。レコードの
	// `ImageScale` を書く道は**実機で効かないと確かめた**——書けて読み戻せるのに OIP も
	// 実描画も既定の 1:50 から動かない（docs/DEV-NOTES.md「打ち切った調査」）。
	//
	// ★**必ず refreshLegends の後に呼ぶ。** 凡例イメージは中身を流し込んだときに作られる
	// ので、作り直しの前には**まだ存在しない**。
	//
	// **単位の取り決めが分からないので読んでから決める**——「分母」（1:50 なら 50）と
	// 「倍率」（0.02）のどちらで持つか SDK ヘッダに書かれていない。既定の 1:50 を読んで
	// 1 より大きければ分母、小さければ倍率と見なし、同じ流儀で書く。読み値は counts へ
	// 持ち帰るので、実機のダイアログで真偽を確かめられる。
	//
	// scale が 0 以下なら何もしない。

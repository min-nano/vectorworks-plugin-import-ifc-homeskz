...
//	  * **イメージの縮率は PIO 既定の 1:50 のまま**にする（既知の制限）。伏図の縮尺は用紙に
//	    合わせて自動で決まる（core::planLayout）ので 1:50 とは限らず、違うと凡例のシンボル
//	    だけが図と大小する。それでも**プラグインからは変えられない**——パラメトリック
//	    レコードの `ImageScale` は書けて読み戻しも変わるのに、OIP の表示も実描画も 1:50 の
//	    まま動かない。VW は OIP →レコードの向きにしか写しておらず、縮率の実体（「凡例
//	    イメージの定義」ダイアログの縮尺と連動する内部状態）に SDK の入口が無い。
//	    タグ付きデータも当たったが、縮率だけを変えた 3 枚の凡例では**8 つの容れ物すべてが
//	    バイト単位で同一**だった（`'ORTC'` のハンドル値を除く）。**ここは調査を打ち切った**
//	    ——伏図が 1:50 でないときは、取り込み後に OIP の「イメージの縮率」で 1 回変えて
//	    いただく（docs/DEV-NOTES.md「打ち切った調査」の「凡例のイメージの縮率を合わせる」）。

...
//	    縮尺ではない。
//	    ★**中身を変えてから作り直す。** イメージの中身は凡例イメージ（＝ビューポート。
//	    `kViewportNode` ＝ 122。実機で凡例の中身の節点型を数えて判明）の縮尺で決まり、
//	    セルの**枠**はその中身に合わせて**作り直しのときに組み直る**。順序を逆にすると枠だけ
//	    既定のまま残る（下の applyLegendImageScale）。
...
namespace HomeskzIfcImport::draw
{
	...
	struct LegendCounts
	{
		...
		// OIP の「イメージの縮率」の表示元（レコードの `ImageScale`）を、すべて終えた後に
		// 読み直した値。**この欄は書かない**（書くと絵が 1:50 へ戻る。draw/Legend.cpp）ので
	...
	// 触らない（docs/DEV-NOTES.md「打ち切った調査」）。枠は縮率の欄からではなく中身から
	// 決まるので、触れなくても困らない——**残るのは OIP の表示が 1:50 のままになることだけ**で、
	// 実描画（中身も枠も伏図の縮尺）には影響しない（README「既知の制限」）。
	//

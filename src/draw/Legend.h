...
//	    書き先はパラメトリックレコードの `ImageScale`（分母を実数で持つ）で、合わせる相手は
//	    **用紙の割り付けが決めた縮尺**（core::planLayout）——デザインレイヤの縮尺ではない。
//	    ★**書くのは作り直しの後**（下の applyLegendImageScale）。
...
namespace HomeskzIfcImport::draw
{
	...
	// 直後の読み戻しは 75 なのに取り込み後の OIP は 1:50 に戻っていた——**作り直しが縮率を
	// 戻す**（docs/DEV-NOTES.md「グラフィック凡例」）。0 以下なら何もしない。

...
//	    docs/DEV-NOTES.md「打ち切った調査」）。**凡例イメージはビューポート**
//	    （`kViewportNode` ＝ 122。実機で凡例の中身の節点型を数えて判明。凡例 1 つに 1 つ）
//	    なので、伏図のビューポートと同じ `ApplyViewportScale` で縮尺を与える
//	    （下の applyLegendImageScale）。凡例イメージは中身を流し込んだときに作られるので、
//	    **必ず作り直しの後**に呼ぶ。
...
namespace HomeskzIfcImport::draw
{
	...
	//
	// **書き先は凡例 PIO のレコードではなく、凡例の中の「凡例イメージ」**——実機で凡例の
	// 中身の節点型を数えたら `0×102, 3×66, 10×6, 11×36, 86×42, 122×12` で、**凡例 1 つに
	// つきビューポート（`kViewportNode` ＝ 122）が 1 つ**入っていた。イメージの縮率とは
	// そのビューポートの縮尺のことなので、伏図のビューポートと同じ `ApplyViewportScale` で
	// 与える。レコードの `ImageScale` を書く道は**実機で効かないと確かめた**——書けて
	// 読み戻せるのに OIP も実描画も既定の 1:50 から動かない

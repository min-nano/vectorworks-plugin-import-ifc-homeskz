...
//	  * **イメージの縮率は合わせられない**（いまは PIO の既定＝1:50 のまま）。凡例に並ぶのは
//	    図に出ているのと同じ部材なので、本来は伏図の縮尺（用紙への収まりが決める。
//	    core::planLayout）に揃えたい。**が、縮率はレコードでは設定できない**——`ImageScale`
//	    へ書いて読み戻しても 75 のまま OIP は 1:50 を表示し、描画も 1:50 だった。VW は OIP を
//	    操作したときにレコードへ書き写しているだけで、描画に使う縮率はレコードから読んで
//	    いない。**同じ誤りを 2 度している**ので、レコードへ書く実装を復活させないこと
//	    （経緯と次の一手は docs/DEV-NOTES.md「グラフィック凡例」）。

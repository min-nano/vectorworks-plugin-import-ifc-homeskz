...
//	    縮尺ではない。
//	    ★**書く先は 2 か所ある。** セルの**枠**の大きさはパラメトリックレコードの
//	    `ImageScale`、イメージの**中身**は凡例イメージ（＝ビューポート。`kViewportNode`
//	    ＝ 122。実機で凡例の中身の節点型を数えて判明）の縮尺。片方だけだと枠と中身が
//	    食い違う。順序も決まっている（レコード → 作り直し → ビューポート）——下の
//	    applyLegendImageScale。
...
namespace HomeskzIfcImport::draw
{
	...
	struct LegendCounts
	{
		...
		// レコードの `ImageScale`（＝セルの枠の大きさと OIP の表示を決める欄）。書けなかった
		// 件数と、**作り直しの後に読み直した値**。枠だけ既定の 1:50 に居残っていないかを
		// 見るためのもので、食い違ったときだけ診断へ出す。
		std::size_t recordLeft = 0;
		double recordScale = 0.0;

		// 【一時計装 ── 縮率の欄が特定できたら消す】レコードの欄の一覧（名前/種別、縮率
		// がらみは値も）。**dev ビルドでのみ埋まる。** OIP の「イメージの縮率」ポップアップが
		// どの欄なのかを実機から一度だけ持ち帰るためのもの（draw/Legend.cpp）。
	...
	//
	// ★**縮率は 2 か所にある**（実機で 1 つずつ確かめた）:
	//   * **セルの枠の大きさ**と OIP の表示 … パラメトリックレコードの `ImageScale`
	//   * イメージの**中身**の縮尺       … 凡例イメージ（＝ビューポート）の縮尺
	// 片方だけでは食い違う。実際に、ビューポートだけへ与えたら**枠は 1:50・中身は 1:75** で
	// 中身が枠の中に小さく浮いた。
	//
	...
	//
	// ★**順序が肝心**——レコードへ書く → **作り直す** → ビューポートへ書く。
	//   * 枠の大きさは**作り直しのときに計算される**ので、レコードへ書いただけでは枠に効かない
	//     （これを落としていたため「レコードは効かない」と読み違えた）。
	//   * 作り直しは中身を作り直す＝ビューポートを既定の縮尺で作り直すので、ビューポートへ
	//     書くのは**その後**でなければならない。
	//   * 全体としては **refreshLegends の後**に呼ぶ（凡例イメージは中身を流し込んだときに
	//     作られるので、それより前には存在しない）。**placeLegends より前**に呼ぶ（枠の
	//     大きさが変わる＝凡例の大きさが変わるので、先に動かすと右上が揃わない）。
	//

...
//	**この値は「シンボルのみ」**である。いま凡例を載せているのは基礎伏図だけで、並べたいのは
//	アンカーボルト（ハイブリッドシンボル）なので用途と合っている——が、**構造材ツールの部材
//	（横架材・柱・垂木）は並ばない**。かつてのスタイルが持っていた `PON='StructuralMember'`
//	系の条件を出したくなったら、その条件で手設定した凡例を実機でダンプして写し直すこと
//	（22 バイトの中身は解けていないので、こちらで組み立てられない）。

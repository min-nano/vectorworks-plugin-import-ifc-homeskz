...
//	（経緯と読み方は SDK リファレンス Findings「Graphic Legends」）。書き込みに要るのは 4 つ:
...
//	（スラブ・壁・データタグと同じ扱い。SDK リファレンス Findings「Graphic Legends」）。
...
//	（SDK リファレンス Findings「Graphic Legends」）。
//
...
//	  * ★**イメージの縮率は触らない**（PIO 既定の 1:50 のまま）。伏図の縮尺に合わせたいところ
//	    だが、**プラグインからは中身しか変えられず、セルの枠は既定のまま残る**——半端に当たった
//	    状態は「全部 1:50 で揃っている」より見た目が悪いので、当てないほうを採る。手で OIP の
//	    「イメージの縮率」を変えれば枠も中身も正しく変わるので、合わせたい利用者はそちらで
//	    （README「既知の制限」）。潰した経路と根拠は SDK リファレンス Findings
//	    「Graphic Legends」の打ち切った調査。**この凡例は将来 独自 PIO へ置き換える方針**
//	    なので、VW 標準の凡例をこれ以上いじらない。

...
namespace HomeskzIfcImport::draw
{
	...
	//
	// 【なぜ最後にもう一度測るのか】実機で「柱オブジェクトは在り、OIP の値も命令どおりなのに
	// 長さ 0 で描かれない」が起きているが、**生成直後の測定では 1 本も潰れていなかった**
	// （round 1）。つまり潰れるとしたら**柱を描いたあと**——他の要素・伏図・軸組図を作る
	// あいだ——になる。ここで測り直せば「いつ潰れたか」が絵を見ずに分かる
	// （docs/DEV-NOTES.md「柱が長さ 0 で描かれる（M27）」）。
	//
	// 潰れていた柱は**同じ指定のまま**解かせ直し（RetryCollapsedMember）、直った本数まで
	// 数えて outDiagnostics へ返す。outNotes には 1 本目の実測（どのパラメータが何を返して
	// いるか）を入れる——平常でも出る記録なので診断ログにだけ出す。

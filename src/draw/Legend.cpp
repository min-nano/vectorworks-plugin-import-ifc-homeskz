...
namespace HomeskzIfcImport::draw
{
	...
	void applyLegendImageScale(LegendCounts& counts, double scale)
	{
		...
		// ★**順序**: 中身 → 作り直し → レコード → **中身をもう一度**。以降は作り直さない。
		//
		...
		//
		// よって「作り直しで枠を組ませ、その後にレコードと中身を入れ直す」のが唯一の並び。
		// **最後に作り直さない**ことが肝で、作り直した瞬間に全部 50 へ戻る。
		//
		// **レコードは中身より先**（作り直しの直後）に書く——`ApplyViewportScale` が中で呼ぶ
		// `Update()` を挟むと、以降のレコード書き込みが OIP へ伝わらなくなる（実機で 2 通りを
		// 比べて確認。下の 3 のコメント）。
		//
		...
		// 3) OIP の「イメージの縮率」（レコード）を合わせる。★**作り直しの直後に書く。**
		//
		// **ビューポートへ書いた後だと OIP に反映されない。** 実機で 2 通りを比べて分かった
		// ——作り直しの直後に書いた回は OIP のポップアップが「カスタム」に変わり、
		// ビューポートへ書いた後に書いた回は「1:50」のまま動かなかった（どちらもレコードの
		// 読み戻しは 75）。`ApplyViewportScale` は中で `Update()` を呼ぶので、それを挟むと
		// 以降の書き込みが OIP へ伝わらなくなる、と読める。
		for (const MCObjectHandle object : counts.objects)
		{
			try
			{

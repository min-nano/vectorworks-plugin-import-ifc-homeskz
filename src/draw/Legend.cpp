...
namespace HomeskzIfcImport::draw
{
	...
	void applyLegendImageScale(LegendCounts& counts, double scale)
	{
		...
		// ★**順序**: 中身 → 作り直し → **中身をもう一度**。以降は作り直さないし、レコードも書かない。
		//
		...
		//
		// よって「作り直しで枠を組ませ、その後に中身を入れ直す」のが唯一の並び。
		...
		// ★**レコードの `ImageScale`（OIP の表示欄）は書かない。書くと枠が壊れる。**
		//
		// 実機で確かめた並び:
		//   * 中身 → 作り直し（レコードは書かない） … 枠も中身も伏図の縮尺で揃う
		//   * そこへレコードを書き足す           … **枠だけ 1:50 に戻る**
		// レコードを書くと VW が枠を組み直し、そのとき使われるのが OIP の「カスタム 1:」が
		// 指す**別の格納場所**（既定の 50 のまま）だから、と読める。
		//
		// その「カスタム 1:」は `ImageScale` ではない——レコードが 75 でも 50 と表示され、
		// **手で 75 を入力しても中身の大きさは変わらず**、保存して開き直しても 50 のまま残る。
		// レコードの外のどこかに永続化されている（docs/DEV-NOTES.md「打ち切った調査」）。
		//
		// したがって OIP の表示は既定のままにして、**絵が正しいほうを採る**。

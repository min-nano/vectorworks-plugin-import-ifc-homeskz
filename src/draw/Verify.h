...
//	           走る。
//
//	【「絵を変えるから本番に残す」を疑う】かつてここには**潰れた材のパスを作り直す自己修復**
//	（`retryWithFreshPath`）が並んでいた。「外すと潰れた材がそのまま残る＝絵が変わる」という
//	理屈だったが、**繕った結果のほうが悪かった**——差し替えると `ResetObject` がバウンドの
//	`fOffset` を書き換え、利用者が階高を編集した瞬間に長さとして表に出る（撤去の経緯は
//	docs/DEV-NOTES.md「柱が長さ 0 で描かれる（M27）」）。**「絵を変える」は「絵を良くする」
//	ではない**ので、囲まない側へ置くときはその変え方が正しいかまで見る。
//

...
//	こちらだけの手当て。
//
//	【用紙の割り付け（M17）】縮尺も用紙上の位置も、**文書全体の平面の広がりと用紙の大きさ**
...
namespace HomeskzIfcImport::draw
{
						   ...
						   std::string* note, const ObjectHandles* memberHandles)
	{
		...
		// M17 用紙の割り付け。**縮尺も位置も全伏図で同じ**にするため、文書全体の平面の
		// 広がり（＝どの伏図にも共通の「建物の大きさ」）から 1 回だけ決める。用紙の大きさは
		// 最初に用意できたシートレイヤから読む（どのシートも同じ用紙という前提。M17）ので、
		...
		for (const PlacedSheet& sheet : placed)
		{
			...
			// M17 用紙の上での位置。**この伏図に映る範囲**（命令の表示レイヤで絞った平面の

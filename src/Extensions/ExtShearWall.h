...
//	    芯へ置くと必ず重なるため。
//	    記号は実物の寸法を表さない表記なので、内法でも縮尺でも割らない。大きさは
//	    **用紙 mm**（紙の上で三角 6×3mm・丸 直径 3mm）で、絵は**用紙基準（縮尺無視）の
//	    シンボル**が持つ（下記 kShearMark*Symbol）。
...
//	    プラグインはハッチングを作らない。CLAUDE.md「既存の図面リソースを作らない」）。
//
...
namespace HomeskzIfcImport
{
	...
	//   「中身はあるのに空に見える」定義になる**（SDK リファレンスの Findings/Symbols.md）。
	//

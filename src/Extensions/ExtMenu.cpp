...
#ifdef VW_DEV_BUILD
		// 【一時計装 ── 役目を終えたら消す】**何かを選んだ状態で**このコマンドを実行した
		// ときだけ働く。**文書内のグラフィック凡例すべて**（パラメトリックレコードの全欄・
		// 補助オブジェクトのタグ付きデータ）をファイルへ書き出し、**インポートはせずに
		// 終わる**。何も選んでいなければ空振りして通常のインポートへ進む
		// （draw/AuxProbe.h）。
		//
...
#ifdef VW_DEV_BUILD
	// 0.【一時計装 ── 役目を終えたら消す】何かを選んだ状態で実行したときは、インポートの
	//    代わりに凡例のダンプを書き出して終わる（上の RunLegendAuxProbe。何も選んで
	//    いなければ素通りする）。

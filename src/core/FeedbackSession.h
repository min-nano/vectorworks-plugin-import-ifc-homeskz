...
namespace HomeskzIfcImport::core
{
	...
	struct FeedbackSession
	{
		...
		// レイヤ」しか消せず、**取り込み前から図面に在ったレイヤ**（実機のテンプレートに
		// ある通り芯の「共通」など）へ描いた分は残る——実機で絵が二重になった。丸ごと戻す
		// 道は 4 本とも塞がっている（SDK リファレンス #23 / #27 / #31 / #39）。
		//
		// **やり方は「別名で保存して開き直す」。** 1 周目に、そのとき開いている図面を
		// この場所へ**別名保存**する——以後これが基準になる。周の終わりには**別の捨て場所へ
		// 保存し直してから閉じる**ので、基準ファイルには前の周の絵が 1 つも書き戻らない
		// （「保存して閉じる」がそのまま「変更を破棄する」になる。未保存の変更がある文書は
		// `CloseDocument()` が false で閉じられない、という実測への答えでもある。
		// SDK リファレンス「Documents」）。**テンプレートを複製する必要は無い**——開いた
		// テンプレートは普通の図面と同じものなので、いま開いている図面を基準にすればよい。
		//

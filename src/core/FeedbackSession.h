...
namespace HomeskzIfcImport::core
{
	...
	struct FeedbackSession
	{
		...
		// **周ごとに開き直す図面の絶対パス**（テンプレート。空なら開き直さない）。M25 の
		// レイヤ削除は「前の周が自分で作ったレイヤ」しか消せず、**取り込み前から図面に
		// 在ったレイヤ**（実機のテンプレートにある通り芯の「共通」など）へ描いた分は残る
		// ——実機で絵が二重になった。毎周ここから開き直せば、レイヤだけでなくクラス・
		// シンボル定義・スタイルの残りも含めて同じ初期状態から始められる（SDK
		// リファレンス Findings「Documents」。ISDK::OpenDocumentPath）。
		//
		// **これは実機テストの周だけの都合である。** 本番の取り込みは開いている図面へ
		// 描くのが仕事なので、この値を見るのは draw/Feedback だけ。
		...
		// **前の周でこの仕組みが開いた図面のパス**（空なら無し）。次の周はこのパスの図面を
		// 探し、**まだ開いていれば保存して閉じてから**新しい周を開く——閉じずに開き続けると
		// 周の数だけ図面が積み上がる。
		//
		// **参照（fFileRef）ではなくパスで覚える。** fFileRef は VectorWorks を再起動すれば
		// 別の図面へ割り当て直されうるので、記憶（ディスクに残る）と突き合わせる鍵にすると
		// **利用者の図面を「前の周のもの」と取り違えて閉じる**恐れがある。パスなら取り違え
		// ようがない。閉じるのに要る fFileRef は、そのつど `GetOpenFilesList` から引き直す。
		//
		// 指すのは**テンプレートそのものではなく、その複製**である（draw/Feedback の
		// openRoundDocument が一時ディレクトリへ複製してから開く）——テンプレートを直接
		// 開くと、保存の拍子に利用者のテンプレートを書き換えてしまう。
		std::string roundDocumentPath;
		...
		// **テンプレートを一度は訊いたか。** M25 でテンプレート（templatePath）を訊くように
		// なる前から続いている往復は、続きの周に入るので 1 周目のダイアログを通らない
		// ——訊く機会が二度と来ない。そこで**人がメニューを押した続きの周に限り**一度だけ
		// 訊き、断られたらこれを立てて二度と訊かない（毎回訊けば、往復から操作を消した
		// 意味が無くなる）。パレットが回す周（無人）では訊かない。
		//
		// **`loop` の直後に置くこと。** 小さい値どうしを離すと構造体に詰め物が増え、
		// clang-tidy の padding 検査が落ちる（.clang-tidy は警告をエラー扱い）。
		bool templateAsked = false;
	};

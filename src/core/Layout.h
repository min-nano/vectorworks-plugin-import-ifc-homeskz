...
//	  * 伏図の用紙には**グラフィック凡例のための右の 1 列**を空ける。空ける幅は
//	    **描画側が実測した凡例の幅**（planLayout の legendWidth）——凡例は図面の内容で
//	    伸び縮みするので定数で決め打ちにしない。凡例とビューポートは重ならない。
...
namespace HomeskzIfcImport::core
{
	...
	// 無ければ 0）。凡例のぶん（legendWidth ＋ 間隔）を右に空けた残りへ図を収める——
	// 0 なら空けない（凡例の無い文書で用紙を余らせない）。
	//
	// **幅を実測で受け取る理由**は上記（凡例は図面の内容で伸び縮みする）。空けた幅より
	// 凡例が広くて図に食い込む、という事故はこれで構造的に起きない。

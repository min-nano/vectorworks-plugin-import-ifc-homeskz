...
//	core/Trace.h
//
//	クラッシュ診断ログ（Python 版 tracing.py 相当）。フェーズの区切りを 1 行ずつ、
//	**書くたびに必ずフラッシュして**ファイルへ残す。ネイティブプラグインは Python 版と
//	違って落ちるときに VectorWorks ごと落ちる——後から何も残らないので、
//	「ログの最終行の直後」が原因箇所になる形にしておくことが唯一の手掛かりになる
//	（ROADMAP.md M15「core/Trace」）。
//
...
namespace HomeskzIfcImport::core::trace
{
	...
	// 開いているログのパス（開いていなければ空）。エラーダイアログで「どこを見ればよいか」を
	// 案内するために要る。

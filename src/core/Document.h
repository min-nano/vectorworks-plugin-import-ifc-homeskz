...
namespace HomeskzIfcImport::core
{
	...
	// 距離を端ごとに持つ（OIP の「始端オフセット」「終端オフセット」）。**負値が材を短くする。**
	// 本プラグインはこれを使って「パスの端点＝接合相手の芯線上の点」「実際の材の端＝相手の面」
	// という持ち方をする。
	//
	...
	// 端部オフセットを入れる前（＝相手の面で止めていたころ）の長さと一致する。
	//
	...
	//   endBound                        … 終端の高さ基準（同上）
	//   startOffset                     … 始端の端部オフセット（mm。負値＝材を短くする。上記）
	...
	// 突き当たる相手のいない端はオフセット 0 で、端点がそのまま材の端になる。
	...
	//   bottomBound                        … 下端の高さ基準
	//   topBound                           … 上端の高さ基準
	//   startOffset                        … 下端の端部オフセット（mm。負値＝材を短くする）
	...
	// elevation + height + endOffset（どちらもオフセットは負値で材を短くする）。

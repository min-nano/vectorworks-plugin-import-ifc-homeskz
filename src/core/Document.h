...
namespace HomeskzIfcImport::core
{
	...
	// 上端は elevation + height。**部材の実体はこの 2 つが決める鉛直パスが作り**、上下端の
	// バインド（bottomBound / topBound）は高さの基準を与える。柱（管柱・通し柱）は下端を
	// 当階・上端を上階（storyOffset=1）の横架材天端（最上階は軒高）へ、小屋束は上下端とも
	// 当階の横架材天端へバインドする。**小屋束の上端 offset は下端と同値**にする（VW の
	// 構造材ツールは上下端 offset 差を部材長へ加算するため、天端相当の値を入れると柱高さが
	// 二重加算される。parse/Column.h 参照）。
	//
	// Python 版キーとの対応:

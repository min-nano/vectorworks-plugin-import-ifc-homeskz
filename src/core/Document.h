...
namespace HomeskzIfcImport::core
{
	...
	//
	// 【高さも勾配もバインドの offset だけで表す】描画側は**パスに Z を持たせない**（平面座標
	// だけを渡し、3D 化は構造材 PIO がバウンドの解決結果から行う。draw/StructuralMember.h
	// 冒頭「パスは 2D で渡す」）。高さバインドはパス由来の部材長へ加算されるため、パスにも
	// 傾斜を持たせると二重に適用される（横架材の登り梁と同じ。draw/Member.cpp 冒頭）。
	//
	// フィールド:
	...
	//
	// ★**軸は勾配座標系で見た footprint の射影範囲をはみ出さない**（M29）。屋根面
	// オブジェクトは**軸を勾配の基準線として図に描く**ので、外へ出たぶんがそのまま図の
	// 広がりになり、伏図が用紙に収まらなくなる（実機で母屋伏図が縦に建物 1 つぶん
	// ＝5,680mm 大きく測られた。docs/DEV-NOTES.md M29）。正確には:
	//   * 軸の 2 点は軒側の端（勾配方向 d の最大）に乗り、軒方向 e では footprint の
	//     射影範囲 [eMin, eMax] **ちょうど**を端から端まで張る
	//   * upslope 定義点は棟側の端（d の最小）で、e は同じ範囲の中央
	//
	// ★**「boundary の xy 外接矩形に収まる」ではない。** footprint が矩形でなければ、
	// (e, d) の外接矩形の角を xy へ戻した点は xy 外接矩形の外に出うる——**三角形の屋根面
	// では軒が 1 頂点に退化する**ので、軒の直線上に長さを持つ線分を取る限り原理的に避け
	// られない（実フィクスチャ「伏図次郎【2階】」にも三角形の屋根面が 4 面ある）。はみ出す
	// 量は footprint 自身の広がりに抑えられており、旧実装のように**射影範囲 1 つぶん余計に
	// 伸びることは無い**——直したのはそこである。
	//
	// 組み立ては parse/Roof が勾配座標系の射影範囲から行い、上の不変条件は
	// tests/ParseRoofTests が軸の向きから (e, d) を復元して実フィクスチャごと押さえている。
	...
	// だけを渡し、3D 化は構造材 PIO がバウンドの解決結果から行う。draw/StructuralMember.h
	// 冒頭「パスは 2D で渡す」）。高さバインドはパス由来の部材長へ加算されるため、パスにも
	// 傾斜を持たせると二重に適用される（draw/Member.cpp 冒頭）。
	//
	// フィールド:
	...
	// 材の高さは height + endOffset（＝ IFC の押し出し Depth）になる。**描画側はこの 2 つを
	// パスへ入れない**——パスは柱が建つ平面座標だけを持ち、高さは上下端をストーリレベルへ
	// バインドした差（bottomBound / topBound）が決める（draw/StructuralMember.h 冒頭
	// 「パスは 2D で渡す」）。elevation / height はそのバウンドの offset を解析側が決める
	// ための値であり、かつ**描き上がりを検算する**ための値でもある（parse/Column.h 参照）。柱（管柱・通し柱）は下端を当階・上端を
	...
	struct Document
	{
		...
		// M28 図面枠（タイトルブロック）のスタイル名。**空なら図面枠を置かない**（既定）。
		// 取り込み設定（core::ImportOptions::titleBlockStyle）をそのまま写したもので、
		// parse/BuildDocument が入れる。
		//
		// 【なぜ命令の列ではなく文書に 1 つか】図面枠は**作ったシートレイヤすべてに 1 つずつ**
		// 置くもので、IFC の中身で数も内容も変わらない。しかも軸組図のシートレイヤは
		// **何枚できるかが描くときにしか決まらない**（SectionSheetCommand と同じ事情）ので、
		// 命令を 1 枚ずつ並べることがそもそもできない。「枚数に依らない指示」だけを持つ、
		// という sectionSheet と同じ形にする（CLAUDE.md「描くときにしか決まらないものは
		// 命令に持たせない」）。
		//
		// 【中身（図面枠が何を書くか）は持たない】それはスタイルが決めるもので、図面の側に
	...
	//
	// ★**これは「モデル mm の余白」であって「用紙 mm の余白」ではない**（M29 で判明した
	// 構造上の限界）。図に映るもののうち、**縮尺に追随しないもの**——通り芯の丸・柱記号・
	// 耐力壁の伏図記号（どれも**用紙基準シンボル**）・データタグ・図面タイトル——が
	// モデル座標で占める幅は「用紙 mm × そのレイヤの縮尺」で、縮尺が大きいほど広がる。
	// 一方この定数は縮尺に依らない固定値なので、**縮尺が上がるほど見込みが足りなくなる**
	// （1/50 なら用紙 6mm ぶんしか見ていないのに、用紙 20mm の伏図記号はモデルで 1000mm
	// ＝用紙 20mm を占める）。**用紙 mm の余白として持ち直すのが筋**だが、そのためには
	// 「記号とタグが用紙で何 mm を占めるか」の実測が要る（どれも描いてみるまで大きさが
	// 決まらない）。実測は実機フィードバックの往復で採る——はみ出した 1 枚目の
	// 「測った外形 vs 割り当てた枠」を診断へ出してある（draw/Sheet・draw/Section）ので、
	// その差が答えになる。**数字が揃うまで縮尺の決め方は変えない**（変えると図そのものが
	// 変わって、前の周と突き合わせられなくなる）。
	...
	// （out は変更しない）。
	//
	// **見るのは座標を持つ命令すべてと、伏図の断面寸法データタグ**（M29。タグは注釈なので
	// デザインレイヤには載らないが図には映る——絞り込みは関連付け先の横架材のレイヤで行う。
	// **軸組図のタグは見ない**＝あちらの注釈空間は平面座標ではない。実装のコメント参照）。
	//

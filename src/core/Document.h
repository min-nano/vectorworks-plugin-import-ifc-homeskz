...
namespace HomeskzIfcImport::core
{
	...
	//
	// **この測って直す仕組みは、VW がタグを勝手に動かすことへの備えでもある**: データタグは
	// 関連付け先のオブジェクトへ吸着する（ローカル確認で、指定した挿入点ではなく横架材の
	// 端部＝挿入点に置かれた）。描画側は最後に実位置を測って目標との差だけ動かすので、
	// VW がどこへ置いたかに依らず同じ結果になる（draw/Tag.cpp「置いた後に測って直す」）。
	//
	// Python 版キーとの対応:

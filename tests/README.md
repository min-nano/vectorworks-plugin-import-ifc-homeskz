...
| `CoreFeedbackSessionTests` | `src/core/FeedbackSession` | 実機フィードバックの記憶（既定は「何もしない」・書いて読んで元に戻る・壊れた行や知らない鍵を飛ばして読み続ける・ファイルの読み書き）。**2 周目からファイル選択と設定ダイアログが消えるのは、この記憶だけによる** |
| `ParseFeedbackTests` | `src/parse/Feedback` | PR コメントの本文（内訳の 1 行表現と**周回どうしの差分**・機械可読の目印・**所見を載せないこと**（別の 1 通として届く）・**次の周は取り込みをもう一度実行して始まると書くこと**・注意を折り畳まないこと・上限での切り詰め）と**匿名化**（同じ入力なら同じ仮名・素のファイル名とユーザー名がどこにも残らないこと） |
...
8. **`FeedbackScriptTests` / `FeedbackScriptTestsPs`** … **実機フィードバックを投稿する**
   ...
   **トークンの探索順**（環境変数 → 保管 → `gh`）、**組み立てた JSON が本文をそのまま
   往復すること**（本文は診断ログと利用者の所見なので、引用符もバックスラッシュも日本語も
   通る）、そして**トークンが出力へ一度も現れないこと**（漏れれば PR コメントや診断ログに
   ...
   **所見のダイアログ（`ask-note`）**も同じハーネスで見ます。差し替えるのは「別プロセスを
   起こす口」だけなので、**すぐ返ること**（ここで待つと図面が固まり、所見を書く目的が
   果たせない）と、その子が実際にやること（尋ねる → 所見を 1 通として投稿する →
   ブラウザで開く／書かなければ投稿しない／失敗したら必ず伝える）の両方を、本物のコードで
   確かめられます。**結果を投稿しなかった周**（`posted=no`）で文言と目印が変わることも
   ここで押さえます——その周について PR に残るのはその 1 通だけなので、落ちると読む側から
   周ごと消えます。**ダイアログのボタンが対象を名乗っていること**（「所見を送る」／
   「所見を書かない」）は、osascript が出すもので振る舞いでは試せないので、
   **そう書いてあることを検査**しています（実機で「送る／送らない」がどちらの送信の話か
   読み取れなかった。`docs/DEV-NOTES.md` M23）。

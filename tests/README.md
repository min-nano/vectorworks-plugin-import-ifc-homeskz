...
| `CoreImportOptionsTests` | `src/core/ImportOptions` | 取り込み設定（役割の表と enum の添字が一致すること・既定名が従来の固定名のままであること・表示名が空でないこと・差し替えと空文字の扱い） |
...
| `ParseFeedbackTests` | `src/parse/Feedback` | PR コメントの本文（内訳の 1 行表現と**周回どうしの差分**・機械可読の目印・**所見を載せないこと**（人がチャットへ直接書く）・**次の周は「実機テストを実行…」をもう一度実行して始まると書くこと**（M25 で本番の取り込みと分けたので、本番の取り込みを頼んでも往復は動かない）・**「取り消し」を頼むのは取り込み前から在ったレイヤへ描いた周だけにすること**（プラグインが取り除けるのは自分が作ったレイヤだけ。ここを黙ると「戻す必要は無い」と読んだ人の図面に前の周が積み上がる）・**取り込みの前に図面へ何をしたかを「図面の状態」の隣へ出すこと**（診断ログは上限で切り詰められるので、そこだけを頼りにすると読めない周が出る。実機 round 2 で実際に落ちた）・**図面が取り込み前へ戻っていたかを毎回載せること**（1 周目に採った基準との引き比べ・レイヤ名は載せないこと）・注意を折り畳まないこと・上限での切り詰め）と**匿名化**（同じ入力なら同じ仮名・素のファイル名とユーザー名がどこにも残らないこと）・**実機テストの結末を実機テスト自身の言葉で言うこと**（取り込みコマンドの完了文言を借りて後ろへ PR の話を足さない。借りると本番の取り込みが PR へ投稿しているように見える）・**診断ログの切り詰めを UTF-8 の文字境界で行うこと**（バイト位置で切ると壊れた UTF-8 が本文へ入り、GitHub が 400「Problems parsing JSON」で弾いてその周の投稿がまるごと落ちる。実機で発生） |
| `CoreFixtureScanTests` | `src/core/FixtureScan` | 回帰テストの対象フォルダの走査（IFC だけを拾う・サブフォルダへ降りる・シンボリックリンクとショートカットを辿る・**同じ実体を 1 つに畳む**・リンクの輪で止まる・件数と深さの上限・**辿れなかったものを黙って落とさないこと**（切れたリンク・IFC でもフォルダでもない先）・同名は実体のパスで並べること・無いフォルダ）と、パスの小物（選んだファイルの親フォルダ・フォルダ内のパスの組み立て）。**ショートカット／エイリアスの解決は偽物を渡す**——本物は OS の API（`src/draw/Shortcut`）でしか動かず Linux の CI では試せないので、core 側は解決を関数で受け取る形にしてある |
| `ParseRegressionTests` | `src/parse/Regression` | 回帰テストの結果と基準（基準ファイルの往復と**本物のファイルへの読み書き**（無くても異常ではないこと・書けない場所では false）・**名前に `=` が入っても壊れないこと**・**改行が CRLF でも秒が数として読めなくても読むのをやめないこと**（基準ファイルは利用者の機械で作られる）・壊れた行と知らない鍵を飛ばすこと・**複数行を 1 行に畳むこと**（基準ファイルは 1 行 1 鍵なので、描画側の注意も取り込みのエラー本文もそのままでは記録を壊す）・突き合わせの分類（同じ／動いた／基準に無し／今回は走らず）・**基準に依らない異常を別に数えること**（基準がその異常ごと記録されていても見逃さない。「基準どおり」だけを見ていると、ずっと壊れたままのものが永久に見えなくなる）・結末だけが変わった場合・**基準が空でも差分を黙らないこと**・走らせる前の問いと結果とログの文言） |
| `ParseImportOptionsTests` | 設定 → 解析の経路 | 設定したシンボル名が**5 つのモジュール**（アンカーボルト・床束・火打・仕口・伏図記号）の命令すべてに届くこと・既定の設定では従来の名前になること（全フィクスチャ通し） |
...
| `CoreJsonTests` | `src/core/Json` | 最小 JSON（整数の見え方・逃がし・**挿入順の保持**・往復・数・`\u` とサロゲート対）と、**壊れた入力を受け付けない・深い入れ子で再帰しない**こと |
| `CoreBridgeTests` | `src/core/Bridge` | MCP ブリッジの受け渡し（要求を**送った順＝名前の昇順**で拾う・拾ったら消す・応答を原子的に書く・生存の印の出入り）と安全弁（id の綴り検査＝スプールの外へ書かせない・壊れた要求を 1 度だけ報告する・残骸の掃除） |
...
ありません**——後者は実機で確かめます。Claude 側の Python サーバは、プラグインと同じ作法で
スプールへ応える代役に対して回します（下記「MCP サーバ」）。
...
2.5 **`FeedbackLoopTests`** … **モードレスの往復の駆動**（`src/FeedbackLoop.cpp`。M24）を、
   ...
   止まる条件（人が止めた・Claude の合図・PR が閉じた／マージされた・入れられなかった・
   殻まで変わった・取り込めなかった・投稿できなかった）と、**確認そのものができなかった
   ときは止めない**こと、合図を探す `since` に直近の投稿の時刻を渡すことを確かめます。
   M25 で 2 件足した——**メニューの周が走っている間は何もしない**こと（パレットはコマンドを
   押した直後に開くので、取り込みの最中も JS のタイマーが Tick を叩きうる。素通しすると
   駆動が 2 周目を始めようとして本体を降ろしにいく）と、**開いた瞬間に「回っていません」と
   出さない**こと（まだ何も投稿できていないが、そこでそう出ると押した人が失敗したと読む）。
   さらに 1 件——**人がメニューを押すまで回さない**こと（記憶が `active` でも、武装する前の
   Tick は本体にも GitHub にも触らない）。パレットの JS タイマーは「閉じる」で隠しても
   止まらないので、記憶を根拠に回すと**誰も押していないのに取り込みが走り出す**（実機で
   起きた。`docs/DEV-NOTES.md` M25「隠れたパレットは止まらない」）。
...
8. **`FeedbackScriptTests` / `FeedbackScriptTestsPs`** … **実機フィードバックを投稿する**
   ...
   `post` / `loop-control`）を、ネットワークとトークンの保管だけ差し替えて検証します。要の検査は 3 つ——
   **トークンの探索順**（環境変数 → 保管 → `gh`）、**組み立てた JSON が本文をそのまま
   往復すること**（本文は診断ログを含むので、引用符もバックスラッシュも日本語も通る）、
   そして**トークンが出力へ一度も現れないこと**（漏れれば PR コメントや診断ログに
   載りうる）。`tests/vw-feedback.test.sh` / `tests/vw-feedback.Tests.ps1`。
   あわせて **bash 3.2（macOS の `/bin/bash`）で動くこと**を構文の側から押さえます——
   スクリプトが**配列を 1 つも書いていない**ことの検査です。3.2 は `set -u` の下で空の
   配列を展開すると即死しますが、ハーネスが走る bash 5 では起きないので、振る舞いの
   テストでは守れません（実機の round 1 で実際に踏んだ。`docs/DEV-NOTES.md` M23）。
...
検証します（`tests/ci-wait.test.sh`）。プラグインのコードではなく開発ツールですが、
守っている性質——**待機は必ず有限時間で終わる**・「まだチェックが登録されていない」や
「新しい push で古い run がキャンセルされた」を green と取り違えない——が 2 度壊れ、
そのたびに「CI は終わっているのにセッションが気付かない」という事故になったため、
回帰テストを ctest ／CI に組み込んであります。締切・ウォッチドッグ・API 連続失敗の
打ち切りといった歯止めは、フェイクの probe で直接動かして確認します。

### 4.5. clang-tidy の結果キャッシュ（開発ツール）

**`TidyCacheScriptTests`** … `scripts/tidy-cache-key.py`（翻訳単位ごとの鍵）と
`scripts/clang-tidy-sdk.sh -c`（その鍵で解析を飛ばす側）を、**合成のリポジトリ**と
スタブの clang-tidy に対して検証します（`tests/tidy-cache.test.sh`）。これも開発ツール
ですが、間違えたときの現れ方が最悪——**鍵に漏れがあると、直した翻訳単位を解析しないまま
CI が緑になる**——なので回帰テストで押さえてあります。

**スタブは本物と同じ「成功時の雑音」を出します**（`72231 warnings generated.` など）。
これが要点です——控える条件を「出力が空なら」と書いていたせいで実機では**1 件も控えられて
おらず**、それでも CI は緑でした（結果は正しく、ただ何も速くならない）。黙るスタブでは
その欠陥がテストを素通りします。

裏返しの性質——**診断を変えないものでキャッシュを外さない**こと——も見ています
（`clang-tidy --version` に混ざる `Host CPU:` のような実行機ごとに変わる行で鍵が動かない
こと）。この仕組みの壊れ方はいつも「緑のまま速くならない」なので、テストの側で押さえる
しかありません。

見ているのは「診断を変えうるものは、すべてキャッシュを外す」ことです: 翻訳単位そのもの・
**推移的に include されたヘッダ**（`.cpp` は名前すら知らない孫ヘッダ。素朴な鍵が
取りこぼす場所）・`.clang-tidy`・コンパイル指令・`VW_SDK_CACHE_KEY`・**ランナーイメージ**
（`ImageOS` / `ImageVersion`。コンパイラの既定の検索パスから来る標準ヘッダの代表）。裏返しの性質も
同じだけ見ています——**診断の出た翻訳単位を「きれい」として控えない**こと、入力を
突き止められない翻訳単位（compile database に無い・綴りがマクロで決まる `#include`）は
必ず解析されること、そして `-c` を渡さなければ従来どおり全部を解析すること。
本物の clang-tidy は要らず（スタブが「どれを解析させられたか」を記録します）、bash と
python3 だけで走ります。

### 5. MCP サーバ（Claude 側）

**`McpServerTests`** … `scripts/mcp/vw-mcp-server.py`（MCP ブリッジの Claude 側）を、
**プラグインと同じ作法でスプールへ応える代役**に対して動かします
（`tests/vw-mcp-server.test.py`）。Vectorworks 側（`src/draw/McpBridge.cpp`）は SDK が要る
ので CI では動かせませんが、**両者の間の約束**（`src/core/Bridge.h` のファイル名の綴り・
原子的な書き方・生存の印）は代役が真似ているので、どちらかを変えればここが落ちます。
...
加えて **スプールの探し方**（`check_spool_search`）を直に見ています。実機で外したのが
正にここで、Claude のデスクトップアプリが `$TMPDIR` を渡さないため候補が `/tmp` だけに
なり、Vectorworks が利用者ごとの一時フォルダへ置いた橋を見つけられませんでした
（[`docs/DEV-NOTES.md`](../docs/DEV-NOTES.md)「探し方は実機で外した」）。macOS では
利用者ごとの一時ディレクトリを引けること・`$TMPDIR` が無くてもそれが候補に入ること・
**`/tmp` を候補にしないこと**（同「当てずっぽうの候補は置かない」）を、それ以外の OS では
その枝が走らないことを確かめます。**持ち主と権限の確認**（他から書けるスプールを
使わない）も、ここで押さえてあります。

**macOS の枝は CI では走りません**（このテストを回すのは Linux のランナーだけ）。
実機で `vw_bridge_status` の「探した場所」を目で確かめるところまでが 1 組です。

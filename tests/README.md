...
| `ParseFeedbackTests` | `src/parse/Feedback` | PR コメントの本文（内訳の 1 行表現と**周回どうしの差分**・機械可読の目印・**所見を載せないこと**（別の 1 通として届く）・注意を折り畳まないこと・上限での切り詰め）と**匿名化**（同じ入力なら同じ仮名・素のファイル名とユーザー名がどこにも残らないこと） |

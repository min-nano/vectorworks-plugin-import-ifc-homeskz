...
裏返しの性質——**診断を変えないものでキャッシュを外さない**こと——も 1 件だけ見ています。
`clang-tidy --version` の出力には `Host CPU: apple-m1` のような**実行機ごとに変わる行**が
あり、それを鍵に入れていたせいで「復元できているのに 1 件も再利用しない」状態を実機で
作りました（結果は正しいので CI は緑のまま。いちばん気付けない壊れ方です）。

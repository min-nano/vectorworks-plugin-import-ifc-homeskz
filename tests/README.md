...
| `ParseImportOptionsTests` | 設定 → 解析の経路 | 設定したシンボル名が**5 つのモジュール**（アンカーボルト・床束・火打・仕口・伏図記号）の命令すべてに届くこと・既定の設定では従来の名前になること（全フィクスチャ通し）・**図面枠スタイル名が命令セット（`Document::titleBlockStyle`）まで届くこと** |
| `CoreProgressTests` | `src/core/Progress` | 進捗の文言整形・フェーズ配分・件数の勘定・中止フラグのラッチ |
| `CoreDrawTimingTests` | `src/core/DrawTiming` | 描画の区間計測（名前ごとの累計と回数・時間の長い順の並びと同着の決定性・負の経過を 0 として積むこと・診断ログへ載せる本文の整形・共有の集計先）。**所要時間そのものは測らない**——実描画はローカルの VectorWorks でしか走らず、時間に依るテストは CI で必ず揺れる |
...
インストール先と `tasklist` は CI では走りません）。**一覧が古いまま**（アプリを Vectorworks
より先に起動した。実機で起きた）の場面も押さえていて、`vw_bridge_status` がプラグインの
道具を並べること・取り直しを 1 度だけ促すこと・`vw_call` で道具に届くことを見ます。

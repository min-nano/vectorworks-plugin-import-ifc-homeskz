...
| `CoreFixtureScanTests` | `src/core/FixtureScan` | 回帰テストの対象フォルダの走査（IFC だけを拾う・サブフォルダへ降りる・シンボリックリンクとショートカットを辿る・**同じ実体を 1 つに畳む**・リンクの輪で止まる・件数と深さの上限・無いフォルダ）。**ショートカット／エイリアスの解決は偽物を渡す**——本物は OS の API（`src/draw/Shortcut`）でしか動かず Linux の CI では試せないので、core 側は解決を関数で受け取る形にしてある |
| `ParseRegressionTests` | `src/parse/Regression` | 回帰テストの結果と基準（基準ファイルの往復・**名前に `=` が入っても壊れないこと**・壊れた行と知らない鍵を飛ばすこと・突き合わせの分類（同じ／動いた／基準に無し／今回は走らず）・**基準に依らない異常を別に数えること**（基準がその異常ごと記録されていても見逃さない。「基準どおり」だけを見ていると、ずっと壊れたままのものが永久に見えなくなる）・結末だけが変わった場合・**基準が空でも差分を黙らないこと**・走らせる前の問いと結果の文言） |

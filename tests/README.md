...
| `ParseFeedbackTests` | `src/parse/Feedback` | PR コメントの本文（内訳の 1 行表現と**周回どうしの差分**・機械可読の目印・**所見を載せないこと**（人がチャットへ直接書く）・**次の周は「実機テストを実行…」をもう一度実行して始まると書くこと**（M25 で本番の取り込みと分けたので、本番の取り込みを頼んでも往復は動かない）・**「取り消し」を頼むのは取り込み前から在ったレイヤへ描いた周だけにすること**（プラグインが取り除けるのは自分が作ったレイヤだけ。ここを黙ると「戻す必要は無い」と読んだ人の図面に前の周が積み上がる）・**図面が取り込み前へ戻っていたかを毎回載せること**（1 周目に採った基準との引き比べ・レイヤ名は載せないこと）・注意を折り畳まないこと・上限での切り詰め）と**匿名化**（同じ入力なら同じ仮名・素のファイル名とユーザー名がどこにも残らないこと） |

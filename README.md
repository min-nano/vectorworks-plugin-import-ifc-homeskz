...
| **回帰テストを実行… (みんなの構造設計支援Dev)** — 指定したフォルダにある過去の物件を 1 件ずつ取り込み、前回の結果と引き比べるコマンド（**開発版だけ**。[開発ガイド](docs/DEVELOPMENT.md)「回帰テスト」） | メニュー |
...
4. **コマンドをワークスペースに追加します:** ツール ▸ ワークスペース ▸ 現在の
   ...
   **MCP ブリッジを開始…**（と、開発版の **実機テストを実行…** /
   **回帰テストを実行…**）は開発・デバッグ用なので、使うときだけ足せば十分です
   （上記「MCP ブリッジ」）。

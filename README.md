...
- **図面枠**（タイトルブロック）は、設定ダイアログで**図面枠スタイル**を選んだときだけ、
  伏図・軸組図の**すべてのシートレイヤ**に 1 つずつ置かれます。用紙の中心に合わせて置かれ、
  ビューポートより背面に入ります。選ばなければ置かれません（既定）。
...
| 図面枠スタイル（任意） | 図面で使っているもの（名前は自由） | 図面枠が置かれない（選択肢にも出ない） |
...
**図面枠は図面にあるスタイルを当てるだけ**で、プラグインはスタイルを作りません。図面枠
スタイルが 1 つも無い図面では、設定ダイアログのその行が選べない状態で開きます。
...
1. **IFC (ホームズ君) 取り込み…** を実行します
   ...
   追加してください）。
2. IFC ファイルを選びます。
3. **設定ダイアログ**で、取り込む要素と配置するシンボル、そして**図面枠のスタイル**を
   選びます。
   - 要素（アンカーボルト 2 種・床束・火打・仕口・伏図記号 2 種）ごとに 1 行あり、
     ...
     **いま開いている図面にあるシンボル**だけで、プラグインオブジェクトのスタイル
     （データタグ・図面ラベル・グラフィック凡例など）は並びません。
   - **いちばん下の行は「図面枠スタイル」**です。ここにチェックを入れてスタイルを選ぶと、
     伏図・軸組図の**すべてのシートレイヤ**に図面枠が 1 つずつ置かれます。候補は図面にある
     **図面枠スタイル**だけです。**既定はチェックなし**（＝図面枠を置かない）なので、
     何も選ばなければ従来どおりの取り込みになります。
...
3. Vectorworks が起動していないときは、Claude に「Vectorworks を起動して」と頼めば
   ...
   開いたままになる見込みですが、まだ確かめていません。** 起動後に繋がらなければ、
   メニューの「MCP ブリッジを表示…」を 1 回実行してください。
...
- **図面枠は用紙の中心に置くだけ**です（枠の中身はスタイル任せ）。図面枠が図番・図面タイトル・縮尺などを自分で
  書く場合、その中身は図面枠スタイル側の設定（シートレイヤの情報を引く欄）に従います。
  プラグインは図面枠の中身を書き換えません。また**図面枠の枠線とビューポートが重なる
  ことがあります**——伏図・軸組図の縮尺と位置は用紙の印刷可能領域から決めていて、図面枠が
  占める分は差し引いていないためです。

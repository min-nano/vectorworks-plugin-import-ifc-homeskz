...
scripts/
  ...
  clang-tidy-sdk.sh         SDK 依存の翻訳単位（src/draw/ ほか）に clang-tidy を
                            かける。1 ランナー内でコア数ぶん並列（-j）＋対象を
                            複数ランナーへ分割（-s I/N）の 2 段（build.yml の
                            tidy-mac / tidy-windows が使う。対象一覧の唯一の定義）
...
増えるため、割合もわずかに動きます。同一コードに対する CI の実測では、行
98.1%（4379/4463）→ 98.0%（4393/4484）、分岐 70.1% → 69.0%、関数は 100% のまま
でした。しきい値（行 🟢≥95%）に対しては十分な余裕があり、分岐は参考扱い（⚪）です。

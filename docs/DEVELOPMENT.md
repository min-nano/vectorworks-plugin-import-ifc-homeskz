...
src/
  ...
  draw/                     Phase 2: VW 描画（SDK 依存）。**まるごと本体に入る**
    ...
    Feedback.{h,cpp}          実機フィードバックの往復（結果のダイアログ → PR へ投稿 →
                              所見は別プロセスへ。待たないし入れもしない）
...
```
   ① 取り込み（1 周目だけ IFC と設定を選ぶ）
        ↓
   ② フィードバックのダイアログ  ← 結果の本文＋宛先 PR＋伏せるかどうか
        ↓
   ③ PR へコメントを投稿（scripts/vw-feedback.*）
        ↓
   ④ **所見のダイアログを別プロセスで出して、待たずに戻る**
      → Vectorworks は完全に操作でき、図面を確かめてから書いて送れる（下記）
        ↓
   ⑤ 直った版が push され、CI が dev プレリリースを出す
        ↓
   ⑥ **メニューで取り込みをもう一度実行する**（人がするのはこれだけ）
      → 殻が尋ねずにその版を入れて本体を読み直し、①へ戻る
        （ファイル選択も設定ダイアログも再起動も出ません）
```

**人の操作は⑥のメニュー 1 回だけ**で、それ以外——どのビルドを入れるか・何を取り込むか・
どの設定で描くか・何を投稿するか——はすべて自動です。

**ビルドを待たないのは意図してのことです。** 以前は⑤をプラグイン自身が待って⑥を無くして
いましたが、待つあいだ進捗ダイアログが Vectorworks を止めてしまい、**その周の絵が見られ
ません**でした（実機 round 3 の所見）。絵を見るために回している往復でそれでは本末転倒
なので、待つのをやめてあります。**完全な無操作と図面の操作を両立させるにはモードレスな
パレット（`IExtensionWebPalette`）が要る**ので、そちらは別途検討します
...
**絵を見てから書くには Vectorworks を操作できなければなりません。** ところが VW の
レイアウトダイアログはモーダル前提で、開いている間は図面を拡大することもレイヤを
切り替えることもできません（モードレスにするには別の拡張種別 `IExtensionWebPalette` が
要ります——[SDK リファレンス「モードレス（非モーダル）なパレット」](https://github.com/min-nano/vectorworks-developer-sdk-reference/blob/main/Findings/Layout%20Dialogs.md)。
実機未確認）。

そこで**所見だけは OS 標準のダイアログを別プロセスで出します**（`vw-feedback` の
`ask-note`。macOS は osascript、Windows は InputBox）。あちらは**自分を起こし直して
即座に返る**ので、プラグインは待たずに戻り、Vectorworks は完全に操作できる状態に
なります。

**戻ってしまう以上こちらへは返せない**ので、尋ねてから投稿するところまでスクリプトの
仕事です。所見は**独立した 1 通**として PR に載ります（目印は
`<!-- homeskz-ifc-feedback-note v1 … -->`）——取り込み結果の本文へ差し込む形にすると、
コメントの組み立てが C++ 側とスクリプト側の 2 か所に割れてしまうためです。書き終えると
投稿済みのコメントがブラウザで開きます。

**所見を書いている間も、書き終えたあとも、図面はいくらでも確かめられます。** プラグイン
はもう何も待っていません。

**「結果を送らない」を選んだ周にも所見だけは訊きます。** 結果を投稿しないと決めた周は、放って
おくと読む側から丸ごと消えます——そこで何を見て、なぜ送らないと判断したのかは、その人に
しか分かりません。ダイアログの文言も変わり（「これがこの周の唯一の記録になります」）、
所見は `posted=no` を付けた 1 通として PR に載ります。**空のまま閉じれば何も投稿されない**
ので、本当に何も残したくないときの逃げ道もそのままです。

Claude 側はコメントを読んで直し、push します。CI が dev プレリリースを出したら、取り込みを
もう一度実行してください——それが次の周の始まりです。
...
以後は「**絵を見て所見を書く → 直った版が出たら取り込みをもう一度実行する**」の繰り返し
です。

**やめたいときは、次の周の頭に出る確認で「往復を終える」を押します**（記憶が消えて、次の
取り込みはファイル選択から始まります）。フィードバックのダイアログの「結果を送らない」は
**その周を投稿しないだけ**で、往復は続きます——報告したくない周が 1 つあっただけでファイルと
設定を選び直す羽目になるのは筋が通りません。

**ボタンは「何を」するのかを名乗ります。** 結果のダイアログは「結果を送る／結果を送らない」、
所見のダイアログは「所見を送る／所見を書かない」、続きの確認は「続ける／往復を終える」。
ただの「送る／送らない」だと**どちらの送信の話か・往復が終わるのかが読み取れません**
（実機でそう読めなかった、という指摘を受けています）。結果のダイアログには、押したあと
何が起きるか（所見のダイアログが出ること・往復は終わらないこと）も 2 行で書いてあります。
...
| `src/draw/Feedback.*` | ダイアログと投稿（SDK 依存）。**待たないし、入れもしない** |
| `src/draw/HostServices.*` | 殻から借りた道具（同梱スクリプトの実行）の置き場所 |
| `scripts/vw-feedback.sh` / `.ps1` | **ネットワークとトークン**、そして**所見のダイアログ**。`token-status` / `login` / `logout` / `find-pr` / `post` / `ask-note`（末尾の `yes`/`no` が「結果を投稿したか」） |
...
1. **メニューで取り込みを実行する。** 次の周はこれで始まります（上記「1 周の流れ」⑥）。
2. 取り込みの前に **「図面を取り込み前の状態に戻してから続ける」** の確認が 1 枚出ます
   （ここの「往復を終える」が往復を終わらせる唯一の入口です）。
   同じ文書へ 2 回描くと前の周の図形が二重に残るためで、プログラムから「取り消し」を
   掛ける手立ては未調査です（SDK の調査は
   [SDK リファレンス](https://github.com/min-nano/vectorworks-developer-sdk-reference)側で
   行う決まり。[`CLAUDE.md`](../CLAUDE.md)）。

1 を無くすにはモードレスなパレット（`IExtensionWebPalette`）が要ります——待っている間も
図面を操作できるのはパレットだけなので、そこへ載せ替えれば「完全な無操作」と「絵を
見られること」が両立します（別途検討。`docs/DEV-NOTES.md` M23）。

...
**「送らない」を選んだ周にも所見だけは訊きます。** 結果を投稿しないと決めた周は、放って
...
**やめたいときは、次の周の頭に出る確認で「やめる」を押します**（記憶が消えて、次の取り込み
はファイル選択から始まります）。フィードバックのダイアログの「送らない」は**その周を投稿
しないだけ**で、往復は続きます——報告したくない周が 1 つあっただけでファイルと設定を選び
直す羽目になるのは筋が通りません。

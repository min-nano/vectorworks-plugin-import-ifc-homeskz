...
src/
  ...
  core/                     フェーズ非依存の土台（SDK も STEP も知らない純粋コード）
    ...
    Foundation.{h,cpp}        基礎の命令（同一仕様ごとの外形多角形・代表値）と、そこから
                              ソリッドを組み立てる純計算・床付け・PIO のレコードへの直列化
    Geometry.{h,cpp}          自前の Vec2 / Vec3 / Mat4（配置行列）と平面幾何の基本演算
                              （面積・内外判定・直線交点・辺ごとのオフセット・矩形クリップ）
    PolygonBool.{h,cpp}       平面多角形の集合演算（和・差）と連結成分・畳み込み

...
scripts/
  ...
  tidy-cache-key.py         その結果キャッシュの鍵を 1 翻訳単位ぶん計算する
                            ...
                            clang-tidy の版・SDK の同一性 をハッシュにまとめる）
...
| 追加の clang-tidy 引数 | `-x` で渡した文字列 |

安全側への倒し方は 3 つあります。**`#include` の走査は `#if` を評価せず、書かれている
ものをすべて辿ります**（多めに拾うのは「使い回せたのに解析した」だけで無害、取りこぼしは
見落としに直結）。**控えるのはきれいに通ったときだけ**で、診断が 1 行でも出た翻訳単位は
控えません（次も同じことを言わせるため）。**入力を突き止められない翻訳単位**（compile
database に無い・綴りがマクロで決まる `#include`）は鍵を出さず、必ず解析します。

**鍵に「実行機ごとに変わるもの」を入れないこと。** これは実機で実際に踏みました
——`clang-tidy --version` の出力を丸ごと鍵に入れていたところ、その中の
`Host CPU: apple-m1` が**ランナーの機種によって変わる**（GitHub のプールは機種が混在
しています）ため、実行のたびに別の鍵になり、**キャッシュは復元できているのに 1 件も
再利用されない**（`0 reused / 21 analysed`）状態になっていました。結果は正しいままなので
**CI は緑で、誰も気付きません**——速さのための仕組みにとっては、これがいちばん質の悪い
壊れ方です。いまは `LLVM version X.Y.Z` の番号だけを取り出しています。

同じことが二度起きても気付けるよう、**控えが復元されたのに 1 件も引けなかった実行は
`::warning::` を出します**。規則（`.clang-tidy`）・SDK・共有ヘッダを変えた実行なら当然
出ますが、そうでないのに出ていたら鍵が壊れています。

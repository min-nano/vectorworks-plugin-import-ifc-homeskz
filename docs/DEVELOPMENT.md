...
```
   ...
   ② 1 周目だけ: IFC を選ぶ → 取り込み設定 → **結果を PR へ送るか**（宛先・伏せ字）
      2 周目以降: **ダイアログは 1 枚も出ません**。前の周が作ったレイヤは
      **プラグインが自分で取り除いてから**描き直します（M25。下記）
        ↓
...
**図面を「取り消し」で戻す必要はありません（M25）。** 前の周が作ったレイヤ（デザイン・
シートとも）をプラグインが名指しで取り除いてから描き直します。**消すのは自分が作ったものだけ**
で、利用者が別の用途で足したレイヤには触れません。プログラムから「取り消し」を掛ける道は
...
**やめたいときは、パレットの「往復を止める」を押します**（記憶は残るので、取り込みをもう
一度実行すれば続きから走ります）。記憶ごと消したいときは、新しいビルドが無いときに取り込みを
...
1. **「取り消し」で図面を取り込み前へ戻す。** 同じ文書へ 2 回描くと前の周の図形が

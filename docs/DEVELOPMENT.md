...
```
   ① 取り込み（1 周目だけ IFC と設定を選ぶ）
        ↓
   ② フィードバックのダイアログ  ← 結果の本文＋宛先 PR＋続け方
        ↓
   ③ PR へコメントを投稿（scripts/vw-feedback.*）
        ↓
   ④ **所見のダイアログを別プロセスで出して、待たずに戻る**
      → Vectorworks が動かせるので、図面を確かめてから書いて送れる（下記）
        ↓
   ⑤ 「自動で続ける」なら、同じブランチの新しい dev ビルドを待つ
      （30 秒ごとに確認・最長 90 分・中止できる）
        ↓
   ⑥ 出たら殻へ返る → 殻が**尋ねずに入れて**本体を読み直し、①へ戻る
      （2 周目からは無操作。再起動も要らない）
```

**入れるのは殻（更新の確認）の仕事です。** 本体（ペイロード）がするのは⑤の「出るまで
待つ」までで、インストールと差し替えは取り込みコマンドの頭にある更新の確認が行います
（`UpdateCheckKind::Auto`。下記）。本体のコードがスタックに載っている間は本体を降ろせ
ないので、差し替えはどのみち殻へ返ってからにしかできず、**インストールの経路はこの
リポジトリに 1 本だけ**に保てます。

### 2 周目以降は尋ねずに更新する（`UpdateCheckKind::Auto`）

取り込みコマンドは頭で更新を確認します（[上記](#更新の確認は起動時ではなく-2-つの入口から)）。
往復の中では、その確認の性格が周によって変わります。

| 周 | 種別 | ふるまい |
| --- | --- | --- |
| 1 周目 | `Silent` | 更新があるときだけ「インストールしますか？」と尋ねる。オフラインなら黙って取り込みへ進む |
| 2 周目以降 | `Auto` | 同じブランチの新しいビルドを**尋ねずに入れ、黙って続ける** |

2 周目以降で尋ねないのは、そこにいるのが**「新しいビルドが出た」と見届けて戻ってきた
直後**だからです。周ごとに確認を挟むのは、この仕組みが無くそうとしている手間そのもの
になります。`Auto` が口を開くのは**輪が止まるとき**だけ——殻まで変わった（再起動が
要るので、勝手に落とさず伝えて止まる）か、入れられなかったときです。
...
**「自動で続ける」を入れているあいだは図面を操作できません**（ビルドを待つ進捗ダイアログ
が Vectorworks を止めます）。じっくり見てから書きたい周は、自動継続を切ってください
——所見のダイアログだけが残るので、好きなだけ図面を確かめられます。

...
**殻**に入るのは「Vectorworks に番地を握られるもの」だけ——メニュー 5 つ（うち 2 つは
...
src/
  ...
  payload/
    PayloadMain.cpp           本体のエントリポイント。GS_InitializeVCOM を自分で呼び、
                              取り込み・実機テスト・回帰テスト・MCP ブリッジ・PIO の
                              リセットを中の実装へ取り次ぐ
  Extensions/               **殻**に残る「登録」だけ（実処理は draw/ 側）
    ...
    ExtRegressionMenu.{h,cpp} 「回帰テストを実行…」メニューコマンドの登録と、本体の
                              draw::runRegressionCommand への取り次ぎ（**dev だけ登録**。
                              M28。下記「回帰テスト」）
  ...
  core/                     フェーズ非依存の土台（SDK も STEP も知らない純粋コード）
    ...
    FeedbackSession.{h,cpp}   実機フィードバックの往復で覚えておく値（1 周目の選択）と
                              その読み書き
    FixtureScan.{h,cpp}       回帰テストの対象フォルダの走査（リンク・ショートカットを
                              辿る。解決そのものは draw/Shortcut が注入する。M28）
  ...
  parse/                    Phase 1: IFC 解析（SDK 非依存）
    ...
    Feedback.{h,cpp}          実機フィードバックの PR コメント本文（内訳・前の周との差分・
                              匿名化）
    Regression.{h,cpp}        回帰テストの結果・基準ファイル・突き合わせ・文面（M28。
                              内訳の比較は Feedback の formatTally* を使い回す）
  ...
  draw/                     Phase 2: VW 描画（SDK 依存）。**まるごと本体に入る**
    ...
    Regression.{h,cpp}        回帰テストの運転（フォルダを決める → 1 件ずつ図面を戻して
                              取り込む → 基準と引き比べる。M28。dev だけの入口）
    Shortcut.{h,cpp}          OS のショートカット／エイリアスの解決（Windows の .lnk は
                              COM、macOS のエイリアスは CoreFoundation）
    DocumentFile.{h,cpp}      図面ファイルの小道具（いま開いている図面のパス・別名保存・
                              開き直し）。**実機テストの周と回帰テストが共有する**
...
## 回帰テスト（`draw/Regression`。開発版だけ）

> **過去の物件を使って通し直し、前回の結果と引き比べます。** 入口はメニューの
> **「回帰テストを実行…」**（開発版だけ）。**実機フィードバックの往復とは別物**で、
> あちらは「1 つの物件を、ビルドが変わるたびに取り込み直して PR へ投稿する」もの、
> こちらは「多数の物件を、1 つのビルドで通して基準と引き比べる」ものです。PR へは
> 何も投稿しません。

### なぜ要るか

このリポジトリは公開されているので、**過去の物件の IFC をリポジトリの中に置けません**
（`tests/fixtures/` に入っているのは公開してよい見本だけです）。一方で退行を一番よく
捕まえるのは「実際にやった物件を通し直すこと」で、しかも `draw/` の実描画は CI では
検証できません（[`CLAUDE.md`](../CLAUDE.md)「テスト方針」）。**だから物件はリポジトリの
外に置いたまま、利用者の実機で回します。**

### 使い方

1. **対象フォルダを 1 つ用意します**（どこでも構いません。例: `~/回帰テスト/`）。
...
3. **そのフォルダの中にある IFC を 1 つ**選びます。**選んだファイルの親フォルダが対象**に
   なります（フォルダ選択ダイアログが SDK にあるかは未確認なので、既存の「開く」
   ダイアログだけで済ませています）。
4. 取り込み設定を決めます（**1 回だけ**。全件に同じ設定を使います）。
5. 「何件を・どれくらいの時間で・基準をどうするか」を確認する 1 枚が出ます。
   **尋ねるのはここまで**——あとは終わるまで何も出ません（1 件 1 分前後 × 件数なので、
   40 分を超えることも普通にあります）。
6. 終わると結果のダイアログが出ます。件ごとの内訳は「ログを表示」で読めます。

**いま開いている図面は作業用に複製され**、1 件ごとにそこへ開き直してから取り込みます
（もとのファイルは変わりません）。走り終えたとき画面に出ているのは、その複製に最後の
1 件を取り込んだ状態です。

### 何を「退行」と見るか

2 つあり、**どちらか片方でも出れば結果に出ます**。

| | 見るもの |
| --- | --- |
| **基準との差** | 前回と件数・結末が違う。直した結果どう動いたかを見るのはこちら |
| **基準に依らない異常** | 結末が「成功」でない（例外・検証に不合格・描き切れなかった・描画側の注意）。**基準がその異常ごと記録されていても数えます**——「基準どおり」だけを見ていると、ずっと壊れたままのものが永久に見えなくなります |

内訳の突き合わせは**実機フィードバックの往復と同じ道具**（`parse/Feedback` の
`formatTally` / `formatTallyDiff`）です。周と周を比べるのも、基準と今回を比べるのも
「内訳がどう動いたか」で、同じ比較を 2 通り書く理由がありません。

### 基準ファイル

対象フォルダの直下に `min-nano_structure-regression.txt` として置かれます（物件と一緒に
動くので、フォルダごと別の機械へ持っていっても基準が付いてきます）。中身は `key=value` の
行で、`file=` が 1 件の始まりです。**消せば、次に実行したときの結果が新しい基準**に
なります。

基準を更新するかは**走らせる前**に選びます（終わったところに確認が待っていると席を
離れられないためで、実機フィードバックの往復と同じ考え方です）。**最後まで走らなかった回
（中止・図面を戻せなかった）では基準を書き換えません**——走っていない件が「消えた」ことに
され、次の回から差分が読めなくなるからです。

### リンクの辿り方

対象フォルダに実体を集める必要はありません。次の 4 つを辿ります。

| | 備考 |
| --- | --- |
| シンボリックリンク（mac / Windows） | `std::filesystem` が透過します |
| ジャンクション（Windows） | 同上 |
| `.lnk`（Windows のショートカット） | COM（`IShellLink`）で解決します。**UI は出しません** |
| Finder のエイリアス（macOS） | CoreFoundation のブックマークで解決します。**拡張子が元のまま**（`.ifc` に見える）なので、`.ifc` にも解決を掛けています |

フォルダを指すリンク／ショートカットなら、その先へ降ります（サブフォルダも辿ります）。
**辿れなかったものは結果のログに残ります**——「あるはずの物件が回っていない」に気付ける
唯一の手掛かりだからです。同じ実体を 2 つの経路が指していたら 1 つに畳みます。
...
古い版と引き比べたくて意図的に入れていることもあります。新しい版で測りたいときは、
先に「アップデータを確認」を押してください。

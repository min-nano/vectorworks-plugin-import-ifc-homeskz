...
```
   ...
   ⑤ 直った版が push され、CI が dev プレリリースを出す
        ↓
   ⑥ ①へ戻る（新しいビルドが尋ねずに入り、ファイル選択も設定も再起動も出ません）
```
...
**ビルドを待たないのも同じ理由です。** 当初は⑤をプラグイン自身が待って⑥を無くして
いましたが、待つあいだ進捗ダイアログが Vectorworks を止めてしまい、**その周の絵が見られ
ません**でした（実機 round 3 の所見）。完全な無操作と図面の操作を両立させるにはモードレスな
パレット（`IExtensionWebPalette`）が要るので、そちらは別途検討します
...
以後は「**直った版が出たら取り込みをもう一度実行する**」の繰り返しです（気付いたことは
Claude とのチャットへ書いてください）。
...
| `scripts/vw-feedback.sh` / `.ps1` | **ネットワークとトークン**。`token-status` / `login` / `logout` / `find-pr` / `post`（ダイアログは持ちません） |
...
### 残っている手作業（2 つ、どちらもダイアログ無し）

1. **メニューで取り込みを実行する。** 次の周はこれで始まります（上記「1 周の流れ」⑥）。
2. **その前に「取り消し」で図面を取り込み前へ戻す。** 同じ文書へ 2 回描くと前の周の図形が
   二重に残るためです。プログラムから戻す手立ては未調査で（SDK の調査は
   [SDK リファレンス](https://github.com/min-nano/vectorworks-developer-sdk-reference)側で
   行う決まり。[`CLAUDE.md`](../CLAUDE.md)。起票済み:
   [min-nano/vectorworks-developer-sdk-reference#23](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/23)）、**確認ダイアログも出しません**——押したか
   どうかは戻したかどうかではないので、確認は嘘をつくからです。頼むのは 1 周目のダイアログ
   で 1 度だけ。**実際にどうだったかは、押した／押さないではなく描画側の実測**として、
   毎回の PR コメントの「図面の状態:」に出ます。判定は**1 周目に採った基準との引き比べ**
   です——取り込み前から在ったレイヤの顔ぶれ（`DrawCounts::existingLayers`）を 1 周目に
   記憶し、次の周でそこに無いレイヤへ描いていたら「前の周が残っている」と分かります。
   **真偽 1 つでは足りません**——テンプレートに「共通」等が最初から在れば 1 周目から
   「既存レイヤへ描いた」になり、戻し忘れと区別できないからです。読む側（Claude）はこの
   1 行を見て、絵の破綻が実装のせいか戻し忘れかを切り分けます（レイヤ名そのものは PR へ
   出さず、枚数と判定だけを載せます）。

**往復のやめ方**: 続きの周が走るのは**追っているブランチに新しい dev ビルドが出たとき**
だけです（記憶した sha と動いているビルドが違うとき）。したがって push が止まれば往復も
止まり、**やめるためのボタンは要りません**。新しいビルドを待っている間に取り込みを実行
すると 1 周目のダイアログが出るので、そこで「送らない」を選べば記憶ごと消えます。

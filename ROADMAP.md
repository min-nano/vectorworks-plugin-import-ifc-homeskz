...
- ✅ **屋根階の床＝ロフト（小屋裏収納）**にも対応。ロフトの標準床レベルは「軒高 + 36mm」の
  ...
  **床仕上げ上端（FL レベル）**、ロフトは**床下地下端（軒高レベル＝横架材天端）**
  （ロフトの FL は仮定値なので確かな構造面を基準にする）。`parse/Story` は床版のある屋根階に
  だけ FL レベル（軒高 +36）とレイヤ `R-FL` を足す（空の FL レイヤを作らない）。
...
`床下地`（24）になっている（＝スラブ下端が横架材天端に一致）。スラブスタイル
（`1F-床スタイル` 等）が階ごとに 1 つでき、床がそれを参照している。2D 表現が床ツールと
異なる点も併せて確認する。スキップフロアの段差が高さの差として

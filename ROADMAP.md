...
1. `parse/` の解析＋`core/` の型が**無 SDK で単体テスト**を通る（実 IFC フィクスチャ）。
   ...
   `test_integration.py` に相当する回帰）を確認する。
2. Python 版 `build_document` 出力との**ゴールデン比較**が通る（土台は M6.5 で用意する。
   M6.5 以前のマイルストーンは手書き期待値でよい）。
3. `draw/` が SDK ビルドでコンパイルできる（SDK API の実在は上記 ci-debug で事前確認）。
...
| **M6 屋根面・屋根組（形状先行②）** | 🟨 進行中 | 建物形状②。屋根版から垂木・野地板を導出し屋根面を確定。**PR #19 でコード実装済み・レビュー中（未マージ）**・無 SDK テスト green。**ローカル目視確認待ち**。|
...
**次の一手:** M6（PR #19）のローカル目視確認 → マージ。続けて M6.5 ゴールデン比較の土台を
入れてから M7 横架材へ進む（基礎 M9 は上部形状と独立なので、M7 と並行に着手してよい）。
...
**状態: PR #19 でコード実装済み・レビュー中（未マージ）。無 SDK テスト green・SDK ビルド
（mac）通過。ローカル目視確認待ち。**
...
## M6.5 — ゴールデン比較の土台（`core/DocumentJson`） ⬜

**目的:** Python 版 `build_document` の出力と C++ の Document を**機械的に突き合わせる**土台を、
最大要素（M7 横架材＝Python 版 556 行＋登り梁 296 行）へ入る前に用意する。ここまでは手書き
期待値でテストしてきたが、要素が増えるほど「Python 版とのズレ」を目で追うのが難しくなる。
**Python: `document.py`（スキーマ）／`tests/` の期待値。**

- ⬜ `core/DocumentJson`: Document → JSON（決定的なキー順・数値の桁を固定）。フェーズ間の
  受け渡しは構造体のまま行い、**JSON 往復は必須にしない**（CLAUDE.md「命令セット」）。
- ⬜ 突き合わせハーネス: 同じフィクスチャに対する Python 版 `build_document` の出力
  （JSON を `tests/golden/` へコミット）と、主要フィールドを**許容誤差付き**で比較する
  無 SDK テスト（`DocumentGoldenTests`）。差分は「どの命令のどのキーが何 mm ずれたか」が
  分かる形で報告する。
- ⬜ 期待される差分（本移植で意図的に変えた箇所）を**明示的な除外リスト**として持つ:
  床＝スラブ化に伴う `styleName`/`components`/`datum`/`elevation` の意味、垂木・野地板の
  高さの渡し方、屋根組レベルを屋根版のある階だけに絞る点。除外リストに無い差分は失敗にする。
- ⬜ 既存マイルストーン（M1/M3/M5/M6）の Document をゴールデンに含め、移植済み範囲の
  ズレが無いことを確認する（回帰の網が M7 以降にそのまま効く）。
...
- ⬜ テスト: `ParseMemberTests` / `ParseNoboribariTests`（`test_ifc_member.py` /
  `test_ifc_noboribari.py`）＋ゴールデン比較（M6.5）。
...
- ⬜ テスト: `test_ifc_column.py` / `test_ifc_column_span.py` を写す＋ゴールデン比較。
...
- ⬜ `draw/Footing`: 壁（生成→全体高さ→壁スタイル）、スラブ（`CreateSlab`→スラブスタイル→
  ...
  ISDK の実在 API を確認する。
- ⬜ テスト: `test_ifc_footing.py` の該当部を写す＋ゴールデン比較。
...
- ⬜ 配筋: レコード `配筋` を立上り・底盤に設定（`Pset_Reinforcement`、鋼種接頭辞除去、
  短辺/長辺→X/Y）。
- ⬜ テスト: `test_ifc_reinforcement.py` ほか＋ゴールデン比較。
...
- Python 版と C++ 版の**ゴールデン比較**（M6.5 の土台）を全フィクスチャで回し、意図的な差分
  以外がないことを確認。除外リストの各項目に「なぜ意図的か」を残す。
...
- **M6.5 → M7 以降**: ゴールデン比較の土台を先に置き、以降の要素は Python 版出力との差分で
  移植ズレを検出する。

...
6. **殻の ID（`VW_SHELL_ID`）が「再起動が要るか」を決める。** `CMakeLists.txt` の
   ...
   （`scripts/vw-install.*`）はここに入れない**——殻に入らないものであり、入れると配置の
   手順を直すたびに再起動を強いることになる（下記「配布物の配置はリリース側が決める」）。
...
ものを、そのまま `Plug-Ins` へ置く**（ファイル名を列挙しない。除くのはインストーラ自身）。

したがって**配布物にファイルを足すとき（減らすとき）に触るのは CI の梱包だけ**で、
スクリプトは変えなくてよい。詳細は
[`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)「自動アップデートの仕組み」。

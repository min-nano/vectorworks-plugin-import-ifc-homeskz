...
1. **「図面の状態:」を見る。** ここが「前の周の図が残ったまま重ねて描きました」なら、
   利用者は前の周を「取り消し」で戻さずに実行している——**絵は二重になっているので、
   絵の破綻をそのまま実装のせいにしない**。数字は戻したかどうかで 1 つも変わらないので、
   この 1 行が唯一の手掛かりである。判定は**1 周目に採った基準（取り込み前に在ったレイヤの
   顔ぶれ）との引き比べ**で、1 周目は「基準にします」としか言わない——**テンプレートに
   もとから在るレイヤを「戻し忘れ」と読み違えない**ため（M23 で一度そう読み違えた）。
   **自動の周（M24）では、利用者がその場にいなければ戻せない**——重なっていて当然の周が
   あることを前提に読む（プログラムからメニューの「取り消し」を起動できるかは調査中。
   SDK リファレンス
   [issue #27](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/27)。
   ISDK の undo API では**できない**ことは Findings「Undo」で確定している）。
...
したがって**頼んでよいのは「（取り消しで図面を戻してから）メニューの『実機テストを実行…』を
もう一度実行してください」だけ**にする——しかもパレットが回っている間はそれすら要らない。
ファイルを選び直す・設定を選び直す・再起動する・ログを貼る——このどれかを頼んでいるなら、
それは仕組みが壊れている合図なので、頼む前に直す。push したら、その周で**何が変わるはずか**を

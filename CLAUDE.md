...
`min-nano_structure`（dev ビルドはそれぞれ `…Dev`）。

**プラグイン 1 つに複数のコマンドと PIO を同梱できるので、機能ごとにプラグインを分けず、
この 1 つへ足していきます。** いま入っているのは次のとおり。

- **IFC (ホームズ君) 取り込み…**（メニュー）… **ホームズ君構造EX** が出力する木造軸組
  工法建築物の IFC をパースし、VectorWorks のネイティブオブジェクトへ変換して配置する。
...
| `docs/DEV-NOTES.md` | **開発メモ**。設計の考え方・ホームズ君 IFC の癖・打ち切った調査（本プラグインの方針）・実装の経緯（M0〜M23） |
...
7. **殻は `MinNanoStructureCore`（core/ + parse/）をリンクしない。** リンクできてしまうと「殻に
   入れてよいもの」の境界が曖昧になる。

### 更新の確認は「起動時」ではなく 2 つの入口から

**`plugin_module_main` でアップデートを確認しない。** 本体だけの更新は再起動なしでその場
から効くので、起動のたびに問う理由が無い。入口は次の 2 つで、違いは
`src/UpdaterHost.h` の `UpdateCheckKind` が表す。

| 入口 | kind | 新しいビルドが無かったとき |
| --- | --- | --- |
| メニュー「アップデータを確認」（`src/Extensions/ExtUpdateMenu.cpp`） | `Manual` | **必ず伝える**（最新です／確認できませんでした） |
| 取り込みコマンドの頭（`src/Extensions/ExtMenu.cpp`） | `Silent` | **黙って取り込みへ進む** |

守ること:

- **取り込み側の確認は `PayloadUse` を作る前に置く。** そこで新しい本体が入れば読み直され、
  その 1 回目の取り込みからもう新しいコードが動く。後ろへ動かすと反映が次回に回る。
- **PIO のリセットからは確認しない。** 取り込み直後に数百回走るので、GitHub を叩けない。
- **再起動は SDK に頼む**（`CloseAllFilesAndQuitVectorworks`）。**これが成り立つのは、確認が
  「VectorWorks が完全に動いている最中」にしか走らないから**——起動時（プラグインの読み込み
  中）に呼ぶと「サポートファイルの読み込みに失敗しました」で落ちる（実機で確認済み。
  `docs/DEV-NOTES.md` M23）。**起動時の確認を復活させるなら、再起動の作りも一緒に戻すこと。**
...
**置き先はプラグイン名のフォルダ 1 つ**（`<Plug-Ins>/min-nano_structure/…`）。そのプラグ
...
- **名前空間**: プラグイン固有コードは単一のトップ名前空間 `HomeskzIfcImport` に
  まとめる（M0 でテンプレートの `SamplePlugin` を置換したもの）。フェーズは入れ子
  名前空間（`::parse` / `::draw` / `::core`）で分ける。**M23 の改名では据え置いた**
  ——図面にも配布物にも現れない内部の綴りなので、改名の巻き添えで全ファイルを書き換える
  価値が無い（`docs/DEV-NOTES.md` M23）。
...
（プラグイン名は `min-nano_structure`）。**ユニバーサル名と拡張機能 UUID は
コマンド・PIO の同一性そのものなので、改名しても付け替えない**（付け替えると利用者の
ワークスペースからコマンドが消え、図面上の既存オブジェクトが孤児になる）。

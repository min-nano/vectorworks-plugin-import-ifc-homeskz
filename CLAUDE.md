...
**往復で待たない・入れない。** `draw/Feedback` に**ビルドを待つコードを書かない**——待つ
...
入れてよい」という 1 ビットだけ（`UpdateCheckKind::Auto`）。
...
**往復をやめるボタンを作らない。** 続きの周が走るのは「記憶した sha と動いているビルドが
違うとき」だけなので、push が止まれば往復も止まる（`draw/ImportCommand.cpp`）。やめる意思は
1 周目のダイアログの「送らない」が受け取る（そこで記憶ごと消える）。
...
**直したら push する。** 次の周は、CI が dev プレリリースを出したあとに**利用者が取り込みを
1 回実行して**始まる（新しいビルドは尋ねられずに入り、ファイル選択も設定も出ない。
`docs/DEVELOPMENT.md`「実機フィードバックの往復」）。**プラグインはビルドを待たない**
——待つと進捗ダイアログが図面を塞ぎ、その周の絵が見られなくなるためで、これは実機で
分かったこと（`docs/DEV-NOTES.md` M23「ビルドを待つのをやめた」）。

したがって**頼んでよいのは「（取り消しで図面を戻してから）取り込みをもう一度実行して
ください」だけ**にする。ファイルを選び直す・設定を選び直す・再起動する・ログを貼る——このどれかを頼んでいるなら、それは

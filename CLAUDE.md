...
**周ごとにテンプレートの複製を開くのが本線、レイヤ削除はその控え。** 上のレイヤ削除は
**取り込み前から在ったレイヤ**（実機のテンプレートにある通り芯の「共通」など）に届かず、
そこへ描いた分は残る——実機で絵が二重になった。そこで実機テストの周は、1 周目に選ばせた
**テンプレートを一時ディレクトリへ複製して開き**、毎周まったく同じ初期状態から始める
（`draw/Feedback` の `openRoundDocument`。使う口は `ISDK::OpenDocumentPath` /
`SaveActiveDocumentPath` / `CloseDocument` / `GetOpenFilesList` / `SwitchToOpenFile`。
[SDK リファレンス「Documents」](https://github.com/min-nano/vectorworks-developer-sdk-reference/blob/main/Findings/Documents.md)）。
守ること:

* **テンプレートそのものを開かない。** 開いた図面はそのファイルに紐づくので、保存の拍子に
  利用者のテンプレートを書き換える。開くのは常に複製。
* **前の周の図面はパスで見分ける**（`core::FeedbackSession::roundDocumentPath`）。
  `fFileRef` を記憶に持ち越すと、VectorWorks を再起動したあとに同じ番号が別の図面へ
  割り当たっていて、**利用者の図面を閉じる**恐れがある。
* **保存してから閉じる。** `CloseDocument` に「保存しない」を伝える引数は無いので、
  変更を書き戻してから閉じる——保存を尋ねるダイアログが出れば、誰も見ていない周がそこで
  止まる。保存できなければ**閉じない**（図面が 1 枚増えるほうが軽い）。
* **開き直せなかったら黙って続けない。** いま開いている図面へそのまま描くと前の周に
  重なり、**数字は揃うのに絵が壊れる**——いちばん読み違えやすい形になる。何が起きたかを
  「準備:」の 1 行に載せて PR コメントへ出す。
* テンプレートを選ばなかった周は従来どおりレイヤ削除で戻す（控えとして残す。上記）。

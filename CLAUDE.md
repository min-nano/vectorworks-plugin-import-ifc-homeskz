...
- **回帰テストを実行…**（メニュー。**dev ビルドだけ**）… **指定したフォルダにある過去の
  物件を 1 件ずつ取り込み、前回の結果（基準）と引き比べる**（M28）。リポジトリは公開されて
  いて物件を中に置けないので、**物件は外に置いたまま利用者の実機で回す**。フォルダには
  シンボリックリンク・ジャンクション・`.lnk`・Finder のエイリアスを並べてよい。**実機
  テストとは別物**——あちらは 1 つの物件を縦（時間）に、こちらは多数の物件を横に見る。
  PR へは何も投稿しない。
...
**回帰テストの入口（`Extensions/ExtRegressionMenu`。dev だけ。M28）はここに入らない**
——**確認しない**のが正しいから。測りたいのは「いま入っているビルド」であって、走らせる
直前に別の版へ入れ替わっては「どの版の数字か」が変わる（古い版と引き比べたくて意図的に
入れていることもある）。新しい版で測りたければ、先に「アップデータを確認」を押せばよい。
...
**回帰テスト（M28）の置き場所は 5 つに割れる。** **走査**（フォルダから IFC を集める・
リンクを辿る・同じ実体を畳む）は `core/FixtureScan`（無 SDK・テストあり）、**ショートカット
／エイリアスの解決**は `draw/Shortcut`（OS の API。Windows は COM、mac は CoreFoundation。
**core へ入れない**——Linux の CI ではコンパイルもできないので、解決の口だけを関数で core へ
渡す）、**結果・基準ファイル・突き合わせ・文面**は `parse/Regression`（無 SDK・テストあり）、
**運転**（フォルダを決める → 1 件ずつ図面を戻して取り込む → 結果を出す）は
`draw/Regression`、**入口の登録**は `Extensions/ExtRegressionMenu`（dev だけ）。**内訳の
比較を作り直さない**——周と周を比べるのも基準と今回を比べるのも「内訳がどう動いたか」なので、
`parse/Feedback` の `formatTally` / `formatTallyDiff` をそのまま使う。**図面を 1 件ごとに
戻す手立ては「作業ファイルを開き直す」1 本だけ**にする——実機テストの 3 段構えは「作業
ファイルを用意できなかったとき」の控えが要るからで、回帰テストは**用意できなければ走らせない**
（無人で何十件も回すものを、汚れた図面から始めても読める数字にならない）。**利用者のものを
消すコードをここに置かない。**
...
壁結合・地中梁・床付け**）は `parse/Footing.h`、**図面ファイルそのものを扱う小道具**（いま開いている図面のパス・同じファイルか・まだ無い
一時パス・別名保存・開き直し）は `draw/DocumentFile`（実機テストの周と回帰テストが共有する

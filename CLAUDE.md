...
`parse/Feedback`、**往復の運転**（結果のダイアログ・投稿）は `draw/Feedback`。
...
書いていない。**人が書いた所見は別の 1 通**として、少し後に届く:

```
<!-- homeskz-ifc-feedback-note v1 round=<N> build=<commit> -->
```

分かれているのは、**絵を見てから書くには Vectorworks を操作できなければならない**から
——プラグインは所見を待たずに戻り、別プロセスのダイアログが尋ねて投稿する
（[`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)「所見は別プロセスのダイアログで書く」）。
**所見が付かない周もある**（見て特に言うことが無かった／書かずに閉じた）。逆に、**内訳の
コメントが無く所見だけ届く周もある**——利用者が「送らない」を選んだ周で、そのときの所見は
`posted=no` を持つ。**その周について PR に載るのはその 1 通だけ**なので、数字が無いことを
理由に軽く扱わない。

読む順序と扱いは:

1. **同じ round の所見を最初に探して読む。** 絵を見られるのは人だけで、そこにしか無い
   情報がここにある。
...
したがって**頼んでよいのは「取り込みをもう一度実行してください」だけ**にする。ファイルを
選び直す・設定を選び直す・再起動する・ログを貼る——このどれかを頼んでいるなら、それは
...
その意思表示はふつう**所見のコメントに書かれる**（「問題なし」「これで良い」等）。件数が

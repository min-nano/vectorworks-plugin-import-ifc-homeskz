...
| `docs/DEV-NOTES.md` | **開発メモ**。設計の考え方・ホームズ君 IFC の癖・打ち切った調査（本プラグインの方針）・実装の経緯（M0〜M22） |
...
**殻が本体へ貸すもの**は 2 つだけ——**殻の ID**（`VW_SHELL_ID`。再起動が要るかの判定に
使う）と**同梱スクリプトの実行**（本体は自分の在り処から同梱物へたどり着けない。読まれる
のは一時ディレクトリの複製だから）。どちらも `VwPayloadHost` に載せ、本体側は
`draw/HostServices` に**写して**持つ（M23）。
...
6. **殻の ID（`VW_SHELL_ID`）が「再起動が要るか」を決める。** `CMakeLists.txt` の
   ...
   （`src/UpdaterParse.h` の `NeedsRestartAfterInstall`）。**インストーラ
   （`scripts/vw-install.*` / `scripts/vw-uninstall.*`）はここに入れない**——殻に入らない
   ものであり、入れると配置の手順を直すたびに再起動を強いることになる（下記「配布物の配置はリリース側が決める」）。
7. **殻は `HomeskzIfcCore`（core/ + parse/）をリンクしない。** リンクできてしまうと「殻に
   入れてよいもの」の境界が曖昧になる。

### 配布物の配置はリリース側が決める

**アップデートで走るスクリプトは、必ず「インストール済みの＝古い」版である。** だから
**同梱スクリプト（`scripts/vw-update.*`）にファイルの配置を書かない**——書くと、構成を
変えるたびに利用者が zip を手で落として置き直す羽目になる（M21 で実際に起きた）。

配置は**配布 zip の直下に同梱されるインストーラ**（`scripts/vw-install.sh` /
`vw-install.ps1`。リリースのアセットとしても公開される）が行い、同梱スクリプトはそれを
呼んで出力を素通しするだけにする。インストーラ側の規則もひとつだけ——**zip の直下にある
ものを、そのまま置く**（ファイル名を列挙しない。除くのはインストーラ自身）。

したがって**配布物にファイルを足すとき（減らすとき）に触るのは CI の梱包だけ**で、
スクリプトは変えなくてよい。

**置き先はプラグイン名のフォルダ 1 つ**（`<Plug-Ins>/HomeskzIfcImport/…`）。そのプラグ
インのものが 1 か所に閉じるので、取り除くのが「フォルダを 1 つ消す」で済む。**渡された先が
既にそのフォルダなら足さない**——プラグインは「いま自分が読み込まれたフォルダ」を渡して
くるので、足すと更新のたびに入れ子が深くなる。この規則（`plugin_dir` / `Get-PluginDir`）は
**インストーラ・アンインストーラ・アップデータの 3 つで同じ**でなければならない。

**入れる前に、前の版をその版自身のアンインストーラ**（`scripts/vw-uninstall.*`。zip に
同梱され、**インストール先へ一緒に置かれる**）**で取り除く**。置く側が新しい版の知識を持つ
のと対で、取り除く側はいま入っている版の知識を持つ。順序は「アーカイブの検査 → 取り除く →
置く」——検査を先にしないと「消しただけで入れられない」状態を作る。**削除は必ず安全弁を
通す**（フォルダ名が一致し、中に殻があるときだけ）。ここは本リポジトリで唯一「利用者の
ディスク上のものを消す」コードで、その歯止めは回帰テストで押さえてある。

詳細は [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)「自動アップデートの仕組み」。
...
**実機フィードバック（M23）**: 実機確認の往復はプラグイン自身が回す（下記「実機
フィードバックの往復」）。置き場所は 3 つに割れていて、**覚えておく値**（1 周目の選択）は
`core/FeedbackSession`、**PR コメントの本文**（内訳・前の周との差分・匿名化）は
`parse/Feedback`、**往復の運転**（ダイアログ・投稿・ビルド待ち・入れ替え）は
`draw/Feedback`。ネットワークとトークンは同梱スクリプト `scripts/vw-feedback.*` が持ち、
C++ 側は書かない（自動アップデートと同じ分担）。殻から借りた道具（同梱スクリプトの実行・
殻の ID）の置き場所は `draw/HostServices` ただ 1 つ。
...
## 実機フィードバックの往復（届いたら何をするか）

dev ビルド（＝PR のビルド）は、**取り込みの結果を自分で PR へ投稿する**。仕組みと利用者
側の手順は [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)「実機フィードバックの往復」に
ある。ここに書くのは**受け取る側（Claude）の作法**だけ。
...
この目印があるコメントは**利用者の Vectorworks が自動生成したもの**で、人が書いたのは
「実機を見ての所見」の節だけである。読む順序と扱いは:

1. **所見を最初に読む。** 絵を見られるのは人だけで、そこにしか無い情報がここにある。
2. **「前の周からの変化」を見る。** 直した結果どう動いたかが 1 行で分かる。数字が動いて
   いないなら、直したつもりのところに届いていない。
3. 要素の内訳・注意・診断ログで裏を取る。

**直したら push するだけでよい。** CI が dev プレリリースを出した瞬間に、その
Vectorworks が自動で取り込み直して次の周を投稿する——**利用者へ「もう一度実行して
ください」と頼まない**（頼むとこの仕組みの意味が無くなる）。push したら、その周で
**何が変わるはずか**を PR に 1〜2 行で返しておく（次のコメントと突き合わせられる）。

**数字だけで判断が付かないときは、確かめてほしい点を返信で挙げる。** 「3 階の梁の天端が
基準面と合っているか」のように**絵で見て答えられる形**にすること。次の周の所見欄に
その答えが返ってくる。

**自動の周は実機確認の代わりにならない。** 描画に触れる PR をマージしてよいのは、
上記「開発プロセス: PR とマージ」2 のとおり**人が「確認できた」と言ったとき**だけで、
その意思表示はふつう**所見の欄に書かれる**（「問題なし」「これで良い」等）。件数が
揃っていることは確認ではない——**命令の数が合っていても絵が破綻していることは普通に
ある**、というのがそもそもこの往復の前提である。

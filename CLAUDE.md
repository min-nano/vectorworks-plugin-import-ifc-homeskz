...
- **実機フィードバックの往復**（モードレスのパレット。**dev ビルドだけ**）… 取り込み結果を
  PR へ送ったあと、新しい dev ビルドが出るたびに入れて同じ条件で取り込み直し、結果を投稿する
  ——Claude が「もう要らない」と合図するか、人がパレットで止めるまで（M24）。
...
から効くので、起動のたびに問う理由が無い。入口は次の 3 つで、違いは
`src/UpdaterHost.h` の `UpdateCheckKind` が表す。

| 入口 | kind | ふるまい |
| --- | --- | --- |
| メニュー「アップデータを確認」（`src/Extensions/ExtUpdateMenu.cpp`） | `Manual` | 尋ねて入れる。**結末を必ず伝える** |
| 取り込みコマンドの頭・1 周目（`src/Extensions/ExtMenu.cpp`） | `Silent` | あるときだけ尋ねる。**無ければ黙って取り込みへ進む** |
| 取り込みコマンドの頭・2 周目以降（実機フィードバックの往復） | `Auto` | **尋ねず・報せず入れる**。入らなかったら輪を止める（`false`） |
...
`parse/Feedback`、**往復の運転**（取り込み前のダイアログ・取り込み後の投稿）は
`draw/Feedback`。
...
と「止めた」の受け口（`vw_payload_loop_end`）だけ（`src/PayloadAbi.h` v3）。
...
メニューから取り込みを実行すれば続きの周として走り、そこでまた回り出す（止めたことが
「最初からやり直し」にならないように）。**新しい停止の入口を足すときも、記憶を消す側へ
倒さない。**
...
**やめる入口はパレットだけに置く。** 手動の周（メニュー）が走るのは「記憶した sha と動いて
いるビルドが違うとき」だけなので、push が止まれば往復も止まる（`draw/ImportCommand.cpp`）。
...
1. **「図面の状態:」を見る。** ここが「前の周の図が残ったまま重ねて描きました」なら、
   ...
   **自動の周（M24）では、利用者がその場にいなければ戻せない**——重なっていて当然の周が
   あることを前提に読む（プログラムから戻す手立ては無い。SDK リファレンス Findings「Undo」）。
...
再起動した）ときは M23 のとおり**利用者が取り込みを 1 回実行して**始まる（新しいビルドは
尋ねられずに入り、ファイル選択も設定も出ない。`docs/DEVELOPMENT.md`「実機フィードバックの
往復」）。**本体はビルドを待たない**——待つのは殻のモードレスなパレットで、モーダルの
...
ある。続きが要るなら「取り込みをもう一度実行してください」と頼む（それで往復も回り直す）。

したがって**頼んでよいのは「（取り消しで図面を戻してから）取り込みをもう一度実行して
ください」だけ**にする——しかもパレットが回っている間はそれすら要らない。ファイルを選び直す・設定を選び直す・再起動する・ログを貼る——このどれかを頼んでいるなら、それは
仕組みが壊れている合図なので、頼む前に直す。push したら、その周で**何が変わるはずか**を
